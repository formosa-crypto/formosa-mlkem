...
def summarize(ps):
    ...
    F, f, FwR2, fwR2 = p2_cyclotomic_error_probability(ps)
    print ("failure: %.1f = 2^%.1f"%(f, log(f + 2.**(-300))/log(2)))
    print ("failure worst case cv: %.1f = 2^%.1f"%(fwR2, log(fwR2 + 2.**(-300))/log(2)))


if __name__ == "__main__":
    # Parameter sets n, m, ks, ke,  q, rqk, rqc, rq2
    ps_light = KyberParameterSet(256, 2, 2, 2, 3329, 2**12, 2**10, 2**3)
