...
Array160 Array168 Array256 Array384 Array1024 Array1408 Array1536 Array1568
Array3168 Array4096 WArray8 WArray16 WArray32 WArray33 WArray34 WArray40
WArray64 WArray128 WArray160 WArray168 WArray192 WArray200 WArray256
WArray384 WArray512 WArray1408 WArray1536 WArray1568 WArray2048 WArray3168
WArray8192.
...
module M = {
  ...
  proc a34____a_ilen_read_upto8_at (buf:W8.t Array34.t, offset:int,
    ...
    if ((((aT < cUR) \/ ((cUR + 8) <= aT)) \/ ((lEN = 0) /\ (tRAIL = 0)))) {
      ...
      if ((8 <= lEN)) {
        w <-
        (get64_direct (WArray34.init8 (fun i => buf.[i])) (offset + dELTA));
        w <@ __SHLQ (w, aT8);
        dELTA <- (dELTA + (8 - aT8));
        lEN <- (lEN - (8 - aT8));
        aT8 <- 8;
        ...
        if ((4 <= lEN)) {
          w <-
          (zeroextu64
          (get32_direct (WArray34.init8 (fun i => buf.[i])) (offset + dELTA))
          );
          w <@ __SHLQ (w, aT8);
          dELTA <- (dELTA + ((8 <= (4 + aT8)) ? (8 - aT8) : 4));
          lEN <- (lEN - ((8 <= (4 + aT8)) ? (8 - aT8) : 4));
        ...
        if (((aT8 < 8) /\ (2 <= lEN))) {
          t16 <-
          (zeroextu64
          (get16_direct (WArray34.init8 (fun i => buf.[i])) (offset + dELTA))
          );
          dELTA <- (dELTA + ((8 <= (2 + aT8)) ? (8 - aT8) : 2));
          lEN <- (lEN - ((8 <= (2 + aT8)) ? (8 - aT8) : 2));
          t16 <@ __SHLQ (t16, aT8);
        ...
        if ((aT8 < 8)) {
          if ((1 <= lEN)) {
            t8 <-
            (zeroextu64
            (get8_direct (WArray34.init8 (fun i => buf.[i])) (offset + dELTA)
            ));
            t8 <- (t8 `|` (W64.of_int (256 * (tRAIL %% 256))));
            dELTA <- (dELTA + 1);
            lEN <- (lEN - 1);
  ...
  proc a1568____addstate_ref (st:W64.t Array25.t, aT:int,
    ...
    aT <- (8 * (aT %/ 8));
    if ((aT8 <> 0)) {
      (dELTA, _LEN, _TRAILB, aT8, w) <@ a1568____a_ilen_read_upto8_at (
      buf, offset, dELTA, _LEN, _TRAILB, aT, aT8);
      st.[(aT %/ 8)] <- (st.[(aT %/ 8)] `^` w);
      aT <- aT8;
    } else {
    ...
    at <- (aT %/ 8);
    while ((at < ((aT %/ 8) + (_LEN %/ 8)))) {
      w <- (get64_direct (WArray1568.init8 (fun i => buf.[i])) offset);
      offset <- (offset + 8);
      st.[at] <- (st.[at] `^` w);
      at <- (at + 1);
    }
  ...
  proc a1568____absorb_ref (st:W64.t Array25.t, aT:int, buf:W8.t Array1568.t,
                            _TRAILB:int, _RATE8:int) : W64.t Array25.t * int = {
    var _LEN:int;
    var iTERS:int;
    var offset:int;
    ...
    _LEN <- 1568;
    if ((_RATE8 <= (aT + _LEN))) {
      (st,  _0, offset) <@ a1568____addstate_ref (st, aT, buf, offset,
      (_RATE8 - aT), 0);
      _LEN <- (_LEN - (_RATE8 - aT));
      aT <- 0;
      (* Erased call to spill *)
      ...
      i <- 0;
      while ((i < iTERS)) {
        (st,  _1, offset) <@ a1568____addstate_ref (st, 0, buf, offset,
        _RATE8, 0);
        (* Erased call to spill *)
        st <@ _keccakf1600_ref (st);
        (* Erased call to unspill *)
    ...
    (st, aT,  _2) <@ a1568____addstate_ref (st, aT, buf, offset, _LEN,
    _TRAILB);
    if ((_TRAILB <> 0)) {
      st <@ __addratebit_ref (st, _RATE8);
    } else {
  ...
  proc __crypto_kem_keypair_jazz (pk:W8.t Array1568.t, sk:W8.t Array3168.t,
    ...
    while ((i < inc)) {
      ...
      (Array3168.init
      (WArray3168.get8
      (WArray3168.set64 (WArray3168.init8 (fun i_0 => sk.[i_0]))
      (((4 * 384) %/ 8) + i) t64)));
      i <- (i + 1);
    }
    (* Erased call to spill *)
  ...
  proc __crypto_kem_dec_jazz (shk:W8.t Array32.t, ct:W8.t Array1568.t,
    ...
    ct, (Array1536.init (fun i_0 => sk.[(0 + i_0)])));
    buf <-
    (Array64.init
    ...
    while ((i < inc)) {
      t64 <-
      (get64 (WArray3168.init8 (fun i_0 => sk.[i_0]))
      (i + ((((((4 * 384) + ((4 * 384) + 32)) + 32) + 32) - (2 * 32)) %/ 8)));
      buf <-
      (Array64.init
      (WArray64.get8
      (WArray64.set64 (WArray64.init8 (fun i_0 => buf.[i_0])) (i + (32 %/ 8))
      t64)));
      i <- (i + 1);
    }
    kr <@ _sha3_512A_A64 (kr, buf);
    ctc <@ __indcpa_enc (ctc, (Array32.init (fun i_0 => buf.[(0 + i_0)])),
    (Array1568.init (fun i_0 => sk.[((4 * 384) + i_0)])),
    ...
    (Array32.init
    (fun i_0 => sk.[((((((4 * 384) + ((4 * 384) + 32)) + 32) + 32) - 32) +
                    i_0)])
    ), ct);
