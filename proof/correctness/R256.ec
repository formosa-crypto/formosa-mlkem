...
op ntt_bound_coefs(coefs : W16.t Array256.t, l u c : int) : bool =
   forall k, l <= k < u => -c*Fq.q <= to_sint coefs.[k] <= c*Fq.q.
...
while (
   ...
   ntt_bound_coefs rp{2} 0 256 (9 - log2 len{1})); last by  auto => />; smt.
wp.
exists* k{1}.
elim* => k1.
move  => l.
...
split; first by smt(@W64).
split; last by smt(@W16 @Array256 @Fq).
have ? : ((to_uint start{2} + to_uint len{2}) < 256);by smt(@W16 @Array256 @Fq).
...
(********* bounding carries *)
(* tighter bound *)
move : H25; rewrite /ntt_bound_coefs => AA. 
 move : (AA (to_uint (j{2} + len{2})) _);
 rewrite to_uintD_small; first 3 by smt(@W16).  
move : H24; rewrite /ntt_bound_coefs => BB. 
 move : (BB (to_uint (j{2})) _); first by smt(@W16).
move => *.
...
move => ??.
rewrite (_:k0 = to_uint j{2} + to_uint len{2}); first by smt().
...
move : H21; rewrite H20.
move :H27; rewrite ultE. move => *.
