...
from CryptoSpecs require import GFq Rq Serialization Symmetric Sampling VecMat InnerPKE768 Correctness768.
...
from JazzEC require import Array2400 Array1184 Array960 Array1152 Array2304 Array1088 WArray32 WArray33 WArray64 WArray1184.

from JazzEC require import Jkem768.

require import MLKEMFCLib.
import Serialization768 VecMat768 PolyVec PolyMat InnerPKE768.
...
proc sample_noise2_jasmin(noiseseed:W8.t Array32.t) : W16.t Array768.t * W16.t Array768.t = {
    ...
    noise2 <- witness;
    nonce <- (W8.of_int 0);
    aux <@Jkem768.M._poly_getnoise ((Array256.init (fun i_0 => noise1.[i_0])),
    noiseseed, nonce);
    noise1 <- Array768.init
            (fun i => if 0 <= i < 256 then aux.[i] else noise1.[i]);
    nonce <- (W8.of_int 1);
    aux <@Jkem768.M._poly_getnoise ((Array256.init (fun i_0 => noise1.[256 + i_0])),
    noiseseed, nonce);
    noise1 <- Array768.init
            (fun i => if 256 <= i < 512 then aux.[i-256]
            else noise1.[i]);
    nonce <- (W8.of_int 2);
    aux <@Jkem768.M._poly_getnoise ((Array256.init (fun i_0 => noise1.[512 + i_0])),
    noiseseed, nonce);
    noise1 <- Array768.init
            (fun i => if 512 <= i < 768
            then aux.[i-512] else noise1.[i]);
    nonce <- (W8.of_int 3);
    aux <@Jkem768.M._poly_getnoise ((Array256.init (fun i_0 => noise2.[i_0])),
    noiseseed, nonce);
    noise2 <- Array768.init
         (fun i => if 0 <= i < 256 then aux.[i] else noise2.[i]);
    nonce <- (W8.of_int 4);
    aux <@Jkem768.M._poly_getnoise ((Array256.init (fun i_0 => noise2.[256 + i_0])),
    noiseseed, nonce);
    noise2 <- Array768.init
         (fun i => if 256 <= i < 512 then aux.[i-256] else noise2.[i]);
    nonce <- (W8.of_int 5);
    aux <@Jkem768.M._poly_getnoise ((Array256.init (fun i_0 => noise2.[512 + i_0])),
    noiseseed, nonce);
    noise2 <- Array768.init
         (fun i => if 512 <= i < 768 then aux.[i-512]
         else noise2.[i]);
...
proc sample_noise3_jasmin(noiseseed:W8.t Array32.t) : W16.t Array768.t * W16.t Array768.t * W16.t Array256.t= {
  ...
  nonce <- (W8.of_int 6);
  noise3 <- witness;
  noise3 <@Jkem768.M._poly_getnoise (noise3,noiseseed, nonce);
...
proc indcpa_keypair_jazz (pk : W8.t Array1184.t, sk : W8.t Array1152.t, seed : W8.t Array32.t) :
    W8.t Array1184.t * W8.t Array1152.t = {
    ...
    skpv <@Jkem768.M.__polyvec_ntt (skpv);
    e <@Jkem768.M.__polyvec_ntt (e);

    aux <@ Aux.inner_product ((Array768.init (fun i_0 => a.[i_0])),skpv);
    pkpv <- Array768.init (fun i => if 0 <= i < 256 then aux.[i] else pkpv.[i]);
    aux <@ Aux.inner_product((Array768.init (fun i_0 => a.[768 + i_0])),skpv);
    pkpv <- Array768.init (fun i => if 256 <= i < 512 then aux.[i-256] else pkpv.[i]);
    aux <@ Aux.inner_product ((Array768.init (fun i_0 => a.[1536 + i_0])), skpv);
    pkpv <- Array768.init (fun i => if 512 <= i < 768 then aux.[i-512] else pkpv.[i]);

    pkpv <@Jkem768.M.__polyvec_add2 (pkpv,e);
    pkpv <@Jkem768.M.__polyvec_reduce (pkpv);

    sk <@ M.__i_polyvec_tobytes(sk, skpv);
    aux_0 <@ M.__i_polyvec_tobytes(Array1152.init (fun (i_0 : int) => pk.[0 + i_0]), pkpv);
    pk <- Array1184.init (fun (i_0 : int) => if 0 <= i_0 < 0 + 1152 then aux_0.[i_0 - 0] else pk.[i_0]);
    _aux <- 32 %/ 8;
    i <- 0;
    while (i < _aux){
  ...
  proc iredcompr(ctp: W8.t Array1088.t,bp:W16.t Array768.t) : W8.t Array960.t = {
    ...
    bp <@Jkem768.M.__polyvec_reduce (bp);
    aux_0 <@Jkem768.M.__i_polyvec_compress (Array960.init (fun (i_0 : int) =>  ctp.[0 + i_0]),
    bp);
    return aux_0;
  }
  ...
  proc iindcpa_enc_jazz (ctp: W8.t Array1088.t, msgp:W8.t Array32.t, pk : W8.t Array1184.t,  noiseseed : W8.t Array32.t) : W8.t Array1088.t = {
    ...
    pkpv <- witness;
    pkpv <@ M.__i_polyvec_frombytes(Array1152.init (fun (i : int) => pk.[0 + i]));
    publicseed <- witness;
    inc <- 32 %/ 8;
    w <- 0;
    while (w < inc){
    ...
    k <- witness;
    k <@Jkem768.M._i_poly_frommsg (k, msgp);

    (sp_0,ep,epp) <@ sample_noise3_spec(noiseseed);

    sp_0 <@Jkem768.M.__polyvec_ntt (sp_0);

    bp <- witness;

    aux <@Jkem768.M.__polyvec_pointwise_acc ((Array768.init (fun i_0 => at.[i_0])),
    sp_0);
    bp <- Array768.init
          (fun i => if 0 <= i < 256 then aux.[i] else bp.[i]);
    aux <@Jkem768.M.__polyvec_pointwise_acc ((Array768.init (fun i_0 => at.[768 + i_0])),
    sp_0);
    bp <- Array768.init
          (fun i => if 256 <= i < 512 then aux.[i-256] else bp.[i]);
    aux <@Jkem768.M.__polyvec_pointwise_acc ((Array768.init (fun i_0 => at.[1536 + i_0])),
    sp_0);
    bp <- Array768.init
          (fun i => if 512 <= i < 768
          then aux.[i-512] else bp.[i]);

    v <- witness;
    v <@Jkem768.M.__polyvec_pointwise_acc (pkpv, sp_0);

    bp <@Jkem768.M.__polyvec_invntt (bp);
    v <@Jkem768.M._poly_invntt (v);

    bp <@Jkem768.M.__polyvec_add2 (bp, ep);
    v <@Jkem768.M._poly_add2 (v, epp);
    v <@Jkem768.M._poly_add2 (v, k);

    v <@Jkem768.M.__poly_reduce (v); 
    ...
    (aux_1,
    aux) <@Jkem768.M._i_poly_compress ((Array128.init (fun i_0 => ctp.[(3 * 320) + i_0])),
    v);
    ctp <- Array1088.init
...
while(ctr{1} = j0{2} /\ 0<= j0{2} <= 256 /\ state{1} = XOF.state{2} /\
  (forall (k0 : int), 0 <= k0 => k0 < 32 => extseed{1}.[k0] = seed{2}.[k0]) /\
  (forall (k0 : int),
...
+ auto => /> &1 &2??????; do split; 1..3: by smt().
  move => ctrl polyl aar => *; split; last by smt().
...
wp; conseq />.
splitwhile{1} ^while : ctr0 < 256.
while{1} (!(pos{1} < 166 /\ ctr0{1} < 256) /\ 
      (cond{1} => pos{1} < 166) /\ #post) (168 - pos{1}).
+ move => &h z; rcondf ^if; auto => /> &m  /#.

while(0<=j0{2}<=256 /\ 0<=k{2}<=168 /\ ctr0{1} = j0{2} /\ buf0{1} = b168{2} /\
    (cond{1} => pos{1} < 166) /\ 
    pos{1} = k{2} /\ k{2} %% 3 = 0 /\
   (forall (k0 : int), 
           0 <=  k0 < j0{2} => 
...
seq 17 6 : (#{/~k{2} < 168}{/~cond{1}}pre /\ to_uint val1{1} = d1{2} /\ to_uint val2{1} = d2{2}); 
...
+ sp 1 0; if; 1: by move => &1 &2; rewrite ultE qE; smt().
  + sp 3 2; if{2}.
    + rcondt{1} 2; 1: by move => *; auto => /> *; rewrite ultE; smt().
      ...
      auto => /> &1 &2 j02 aar rpl 8?; rewrite ultE /= => *; do split.
      + smt().
      + smt(). 
      + move => k0 ??.
        case (k0 < j02); 1: by move => *; rewrite !set_neqiE /#.
        move => *;rewrite !get_setE 1..4:/#.
      ...
      + move => *; rewrite set_neqiE 1,2:/# set_eqiE 1,2:/#.
        by rewrite to_sint_unsigned; rewrite /to_sint /smod /=; smt(W16.to_uint_cmp).
      move => *;have -> : (k0 = j02 + 1) by smt(). 
      rewrite set_eqiE 1,2:/#.
      by rewrite to_sint_unsigned; rewrite /to_sint /smod /=; smt(W16.to_uint_cmp).
     
    sp 1 0;if{1}; last first. 
    + auto => /> &1 &2 j02  aar rpl; rewrite !ultE /= => *;  do split.
      + smt().
      + smt().
      + move => k0??;case (k0 < j02); 1: by move => *; rewrite !set_neqiE /#.
        move => *;have -> : (k0 = j02) by smt(). 
  ...
  + rcondt{1} 1; 1: by move => *; auto => /> *; rewrite ultE /#.
    ...
    auto => /> &1 &2 8?; rewrite ultE /= => *; do split.
    + by smt().
    + by smt().
    + move => k0 ??;  case (k0 < j0{2}); 1: by move => *; rewrite !set_neqiE /#.
      move => *;have -> : (k0 = j0{2}) by smt(). 
      rewrite set_eqiE 1,2:/# set_eqiE 1,2:/#.
      by rewrite to_sint_unsigned; rewrite /to_sint /smod /=; smt(W16.to_uint_cmp).
    move => k0 ??; case (k0 < j0{2}); 1: by move => *; rewrite !set_neqiE /#.
    move => *;have -> : (k0 = j0{2}) by smt(). 
...
equiv get_noise_sample_noise :
  Jkem768.M._poly_getnoise ~ CBD2.sample :
...
while(#pre /\ i{1} = i{2} /\ 0 <= i{2} <= 128 /\ j{2} = 2*i{2} /\
       (forall k, 0<=k<2*i{2} =>
...
equiv auxkg_good :
 Jkem768.M.__indcpa_keypair ~ AuxMLKEM.indcpa_keypair_jazz :
     ={arg}
     ==> ={res}. 
proc => /=. 
swap {1} 8 10.
...
seq 13 7 : (#pre /\ ={publicseed, noiseseed}).
+ swap{1} 4 8. swap {1} 4 7.
  seq 8 2 : (#pre /\ ={buf}); last  by sim; auto => />.   
...
transitivity {1} {(skpv,e) <@ AuxMLKEM.sample_noise2_jasmin(noiseseed);}
     (={publicseed,noiseseed,sk,pk,aa}  
         ==> 
           ={sk,publicseed,noiseseed,skpv,e,aa,pk})
     (={pk,sk,publicseed,noiseseed,pkpv} /\ aa{1} = a{2}
        /\ noiseseed{1} = noiseseed{2}  ==> 
          sk{1} = sk{2} 
...
equiv auxienc_good :
 Jkem768.M.__indcpa_enc ~ AuxMLKEM.iindcpa_enc_jazz :
     ={arg} ==> ={res}. 
proc. 
swap {1} 7 -6.
swap {1} 14 -12.
...
swap {1} [14..15] -12.
seq 3 3 : (#pre /\ aat{1}=at{2}); 1: by call auxgenmatrix_good;auto => />.
...
lemma mlkem_correct_kg   : 
   equiv [Jkem768.M.__indcpa_keypair ~ InnerPKE768.kg_derand : 
       randomnessp{1} = coins{2} 
        ==> 
       let (pk,sk) = res{2} in let (t,rho) = pk in
         sk = Array1152.init (fun i => res{1}.`2.[i]) /\
         t = Array1152.init (fun i => res{1}.`1.[i])  /\
         rho = Array32.init (fun i => res{1}.`1.[i+1152]) ].
proc*.
transitivity {1} { r <@ AuxMLKEM.indcpa_keypair_jazz(pk, sk, randomnessp);} 
(={pk,sk,randomnessp} ==> ={r}) 
(   randomnessp{1} = coins{2} 
        ==> 
       let (pk,sk) = r{2} in let (t,rho) = pk in
         sk = Array1152.init (fun i => r{1}.`2.[i]) /\
         t = Array1152.init (fun i => r{1}.`1.[i])  /\
         rho = Array32.init (fun i => r{1}.`1.[i+1152]) ); 1,2: smt().
...
swap {2} [6..7] -5.
seq 2 2 : (#pre /\ a{2} = lift_matrix a{1} /\
            pos_bound2304_cxq a{1} 0 2304 2).
+ inline AuxMLKEM.__gen_matrix; conseq />.
  seq 6 2 : (a0{1}=a{2}); last by auto => />;  smt(matrix_unlift).
...
+ wp 6 3; while{1} (0<=i{1} <= _aux{1} /\ _aux{1} = 4 /\
            rho{2} = publicseed{1} /\
             sv{2} = Array1152.init (fun i => sk0{1}.[i]) /\
             tv{2} =  Array1152.init (fun i => pk0{1}.[i])  /\ 
                forall k, 0<=k<i{1}*8 =>
                 rho{2}.[k] = pk0{1}.[1152 + k]) 
             (4 - i{1}).
...
+ move => *;
  ...
  + rewrite tP => k kb.
    ...
    by rewrite /get8 initiE 1:/# /=.
  + move => k ??.
    rewrite initiE 1:/# /=.
    rewrite get8_set64_directE 1,2:/#/=.
    case ((i{hr} + 144) * 8 <= 1152 + k < (i{hr} + 144) * 8 + 8)=>*; last  by  rewrite /get8 initiE 1:/# /= /#.
     rewrite wordP => i ib.
     rewrite get64E /= /(\bits8) initiE 1:/# /= /pack8_t initiE 1:/# /= initiE 1:/# /= initiE 1:/# /= /#.
  + by smt().
     
  
  wp; ecall {1} (polyvec_tobytes_corr  pkpv{1}). 
...
lemma mlkem_correct_enc _pkp : 
   equiv [Jkem768.M.__indcpa_enc ~ InnerPKE768.enc_derand: 
     (* valid_ptr _pkp (384*3 + 32) /\
     valid_ptr _ctp (3*320+128) /\ *)
     msgp{1} = m{2} /\ 
     pk{1} = _pkp /\
     noiseseed{1} = coins{2} /\
     pk{2}.`1 = Array1152.init( fun i => _pkp.[i]) /\
     pk{2}.`2 = Array32.init( fun i => _pkp.[3*384+i])
       ==> 
     let (c1,c2) = res{2} in
     c1 = Array960.init(fun i => res{1}.[i]) /\
     c2 = Array128.init(fun i => res{1}.[960+i]) 
].
proc*.
transitivity {1} { r <@ AuxMLKEM.iindcpa_enc_jazz(ct,msgp,pk,noiseseed);} 
(={msgp,pk,noiseseed,ct} ==> ={r}) 
  (msgp{1} = m{2} /\ 
     pk{1} = _pkp /\
     noiseseed{1} = coins{2} /\
     pk{2}.`1 = Array1152.init( fun i => _pkp.[i]) /\
     ...
     c2 = Array128.init(fun i => r{1}.[960+i])); 1,2: smt().
...
wp; ecall {1} (polyvec_frombytes_corr 
   (Array1152.init  (fun (i0 : int) => pk0{1}.[0 + i0]))).
+ auto => />  ??rbound;split;last by smt().
  pose xx := (Array1152.init ("_.[_]" _pkp)).
  ...
  have /=? /#:= decode_rng (to_list xx) 12 k.

seq 4 0 : (#{/~pk{1}}pre /\ rho{2} = publicseed{1}). 
while {1} ((tv{2}, rho{2}) = pk0{2} /\ rho{2} = Array32.init (fun (i0 : int) => pk0{1}.[3 * 384 + i0]) /\
           0 <=  w{1} <= 4 /\ inc{1} = 4 /\
          forall k, 0 <= k < w{1} * 8 => rho{2}.[k] = publicseed{1}.[k]) (4 - w{1}).
+ move => &m ?; auto => /> &hr ??H?;do split; 1,2,4:smt().
  move => k kbl kbh;rewrite !initiE 1,2:/# /=.
  case (0<=k <w{hr}*8).
  + move => *; move : (H k _); 1: smt().
    rewrite !initiE 1:/# /= get8_set64_directE 1,2:/# /=.
    case (8 * w{hr} <= k && k < 8 * w{hr} + 8).
    + move => *; rewrite /get64_direct /init8 wordP => ??.
      by rewrite /(\bits8) initiE 1:/# /= /pack8_t  initiE 1:/# /= initiE 1:/# /= initiE 1:/# /= /#.
    by move => *; rewrite /get8 /init8 initiE /#.
  + move => *.
    rewrite  get8_set64_directE 1,2:/# /= ifT 1:/#.
    rewrite /get64_direct /init8 wordP => ??.
    rewrite /(\bits8) initiE 1:/# /= /pack8_t  initiE 1:/# /= initiE 1:/# /= initiE 1:/# /= /#. 

auto => />;move => *;split; 1: smt().
move => pseedl wl;split; 1:smt().
...
+ inline AuxMLKEM.__gen_matrix; conseq />.
  ...
  while (i{1} = i{2} /\ 0<=i{1}<=kvec /\ seed{1}=rho{2} /\ trans{1} /\
         forall ii jj, 0<=ii<i{1} => 0<= jj <3 => (a{1}.[ii,jj])%Matrix = (aT{2}.[ii,jj])%Matrix); last
          by auto => /> *; apply: eq_matrixP; smt().
  wp; while (i{1} = i{2} /\ j{1} = j{2} /\ 0<=i{1}<kvec /\ 0<=j{1}<=kvec /\ seed{1}=rho{2} /\ trans{1} /\
         (forall ii jj, 0<=ii<i{1} => 0<= jj <3 => (a{1}.[ii,jj])%Matrix = (aT{2}.[ii,jj])%Matrix) /\
         (forall jj, 0 <= jj <j{1} => (a{1}.[i{1},jj])%Matrix = (aT{2}.[i{1},jj]))%Matrix); last 
            by auto => />  /#.
  wp; call(_: ={XOF.state}); 1: by sim.
  wp; call(_: ={arg} ==> ={XOF.state}); 1: by sim. 
...
+ inline AuxMLKEM.sample_noise3_spec AuxMLKEM.sample_noise2_spec. 
  wp; call(_: true); 1: by sim.
  conseq />; 1: by smt().
  seq 9 7 : (noise1{1}=rv{2} /\ noise2{1} = e1{2} /\ _N0{1} = _N{2} /\ 
...
seq 18 3 : (u{2} = lift_polyvec bp0{1} /\ v{2} = lift_array256 v{1} /\
            ...
            pos_bound768_cxq bp0{1} 0 768 2); last first.
wp; ecall {1} (i_poly_compress_corr v{1}).
wp; ecall {1} (i_polyvec_compress_corr bp0{1}).
auto => /> &1 &2; rewrite /signed_bound_cxq /signed_bound768_cxq.
move => ??H;split. 
+ by rewrite tP => i ib; rewrite initiE 1:/# /= initiE 1:/# /= initiE 1:/# /= ifF 1:/# initiE 1:/# /= ifT 1:/# initiE 1:/# /=.
+  rewrite tP => i ib; rewrite initiE 1:/# /= initiE 1:/# /= initiE 1:/# /= ifT 1:/# /=.
   by rewrite tP in H; rewrite H 1:/# /compress_poly /encode4 get_of_list 1:/#.  
...
lemma mlkem_correct_dec  : 
   equiv [Jkem768.M.__indcpa_dec ~ InnerPKE768.dec : 
     sk{1} =  sk{2} 
     /\
     let (c1,c2) = cph{2} in
...
swap {1} 2 -1.
swap {1} 3 -2.
seq 1 3 : (#pre /\ s{2} = lift_polyvec skpv{1} /\
...
+ wp; ecall{1}(polyvec_frombytes_corr sk{1}).
  ...
  move => k kb; have := rbound k kb;rewrite  /= /decode12_vec !get_of_list 1:/#.
  by have /=?  /#:= decode_rng ( (to_list sk{2})) 12 k.
...
ecall{1}(poly_tomsg_corr  mp{1}); auto => /> /#.
qed.
