...
from Spec require import Correctness768.
...
proof. 
    ...
    rewrite of_intwE /= /int_bit dvdz_mod_div 1:/#;1,2:    smt(StdOrder.IntOrder.expr_gt0 pow2_8 dvdz_exp2l). 
...
equiv [
Jkem768_avx2.M.__gen_matrix_buf_rejection_filter48 ~ Filters.filter48 : 
  pol{1} = _p
...
hoare [
 Jkem768_avx2.M.__gen_matrix_buf_rejection_filter48
 :  counter = _ctr
...
lemma buf_rejection_filter48_ll:
 islossless Jkem768_avx2.M.__gen_matrix_buf_rejection_filter48
 by islossless.

lemma buf_rejection_filter48_ph _pol _ctr _buf _buf_offset:
phoare  [Jkem768_avx2.M.__gen_matrix_buf_rejection_filter48
 : counter = _ctr
  /\  W64.to_uint _buf_offset + 56 < 536 
   /\ to_uint _ctr <= 256-32
   /\ pol = _pol
...
equiv [
Jkem768_avx2.M.__gen_matrix_buf_rejection_filter24 ~ Filters.filter24 : 
  pol{1} = _p
...
hoare buf_rejection_filter24_h _pol _ctr _buf _buf_offset:
 Jkem768_avx2.M.__gen_matrix_buf_rejection_filter24
 : counter = _ctr
...
+ move => &1 &2 /=.
  ...
  + move :Hs1.
    have ->: (min 256 (to_uint res{1}.`2)) = to_uint _ctr + size (take (256 - to_uint _ctr) yy).
    rewrite Hs2 Hl2 /xx. by smt( size_take size_map take_oversize size_ge0 count_size size_filter size_bytes2coeffs W64.to_uint_cmp size_drop Array536.size_to_list).
    move => ->.
    congr. 
    apply (eq_from_nth witness).
...
lemma buf_rejection_filter24_ll:
 islossless Jkem768_avx2.M.__gen_matrix_buf_rejection_filter24
 by islossless.
