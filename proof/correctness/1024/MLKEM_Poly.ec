...
+ rewrite -compress_impl5 //=.
  rewrite to_sintB_small /=;1: by rewrite  /(to_sint (W16.of_int 3329))  W16.of_uintK /= /smod /=;smt(size_map size_iota).
  rewrite of_sintK /= /smod /= qE; have := H00 ((8 * i + k) %/ 5) _; by smt(W16.to_uint_cmp pow2_16).
  rewrite /truncateu32_4 of_uintK /=. 
  by rewrite /srl_32 /sll_32 /(`<<`) /(`>>`) /= /#.
...
proc change ^while.2: { t0 <- W16_sub t0 (W16.of_int 3329);}; 1: by auto.
proc change ^while.4 : {b <- sra_16 b (W16.of_int 15);}; 1: by auto.
proc change ^while{2}.^while.3 : {d0 <- sll_32 d0 (W32.of_int 5);}; 1: by auto.
proc change ^while{2}.^while.6 : {d0 <- srl_32 d0 (W32.of_int 27);}; 1: by auto.
proc change ^while{2}.5 : { c1 <- sll_8 c1 (W8.of_int 5);}; 1: by auto.
proc change ^while{2}.9 : { c0 <- srl_8 c0 (W8.of_int 3);}; 1: by auto.
proc change ^while{2}.11 : { c1 <- sll_8 c1 (W8.of_int 2);}; 1: by auto.
proc change ^while{2}.13 : { c2 <- sll_8 c2 (W8.of_int 7);}; 1: by auto.
proc change ^while{2}.18 : { c0 <- srl_8 c0 (W8.of_int 1);}; 1: by auto.
proc change ^while{2}.20 : { c1 <- sll_8 c1 (W8.of_int 4);}; 1: by auto.
proc change ^while{2}.24 : { c0 <- srl_8 c0 (W8.of_int 4);}; 1: by auto.
proc change ^while{2}.26 : { c1 <- sll_8 c1 (W8.of_int 1);}; 1: by auto.
proc change ^while{2}.28 : { c2 <- sll_8 c2 (W8.of_int 6);}; 1: by auto.
proc change ^while{2}.33 : { c0 <- srl_8 c0 (W8.of_int 2);}; 1: by auto.
proc change ^while{2}.35 : { c1 <- sll_8 c1 (W8.of_int 3);}; 1: by auto.
...
lemma poly_decompress_corr_h (_a : W8.t Array160.t): 
    hoare [Jkem1024.M._i_poly_decompress  :
             ...
             pos_bound256_cxq res 0 256 1 ].
proc; inline *.
proc change ^while.4 : { c0 <- srl_8 c0 (W8.of_int 5);}; 1: by auto.
proc change ^while.7 : { c1 <- sll_8 c1 (W8.of_int 3);}; 1: by auto.
proc change ^while.12 : { c1 <- srl_8 c1 (W8.of_int 2);}; 1: by auto.
proc change ^while.14 : { c1 <- srl_8 c1 (W8.of_int 5);}; 1: by auto.
proc change ^while.17 : { c0 <- sll_8 c0 (W8.of_int 1);}; 1: by auto.
proc change ^while.22 : { c1 <- srl_8 c1 (W8.of_int 4);}; 1: by auto.
proc change ^while.25 : { c0 <- sll_8 c0 (W8.of_int 4);}; 1: by auto.
proc change ^while.30 : { c1 <- srl_8 c1 (W8.of_int 1);}; 1: by auto.
proc change ^while.32 : { c1 <- srl_8 c1 (W8.of_int 5);}; 1: by auto.
proc change ^while.35 : { c0 <- sll_8 c0 (W8.of_int 2);}; 1: by auto.
proc change ^while.39 : { c2 <- srl_8 c2 (W8.of_int 3);}; 1: by auto.
proc change ^while.^while.5 : {d0 <- srl_32 d0 (W32.of_int 5);}; 1: by auto.
...
conseq (: _ ==> rp = init_256_16 (fun i =>
     decompress5_circuit (W5.init (fun (j : int) => _a.[(i*5 + j) %/ 8].[(i*5 + j) %% 8]))));
      last by circuit.
...
+ auto => /> &1; rewrite /signed_bound_cxq=>  4?lenpow2  2? sval l0 l0l l0h l0v cbdloose cbdtight /= HH=> />;split.
  ...
  auto => />; split.
  + have : 0 < 2 ^ l0 by smt().
    have : 0 <= zetasctr{1} - zetasctr1 by have := pmulr_rge0 (2 * 2 ^ l0) (zetasctr{1} - zetasctr1); smt().
    move: rp. clear. smt().
