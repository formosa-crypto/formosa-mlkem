...
import Serialization1024 VecMat1024 PolyVec PolyMat InnerPKE1024.
...
proc sample_noise2_jasmin(noiseseed:W8.t Array32.t) : W16.t Array1024.t * W16.t Array1024.t = {
    ...
    noise2 <- witness;
    nonce <- (W8.of_int 0);
    aux <@Jkem1024.M._poly_getnoise ((Array256.init (fun i_0 => noise1.[i_0])),
    noiseseed, nonce);
    ...
    nonce <- (W8.of_int 2);
    aux <@Jkem1024.M._poly_getnoise ((Array256.init (fun i_0 => noise1.[512 + i_0])),
    noiseseed, nonce);
    noise1 <- Array1024.init
            (fun i => if 512 <= i < 768
            then aux.[i-512] else noise1.[i]);
    nonce <- (W8.of_int 3);
    aux <@Jkem1024.M._poly_getnoise ((Array256.init (fun i_0 => noise1.[768 + i_0])),
    noiseseed, nonce);
    noise1 <- Array1024.init
            (fun i => if 768 <= i < 1024
            then aux.[i-768] else noise1.[i]);
    nonce <- (W8.of_int 4);
    aux <@Jkem1024.M._poly_getnoise ((Array256.init (fun i_0 => noise2.[i_0])),
    noiseseed, nonce);
    ...
    nonce <- (W8.of_int 6);
    aux <@Jkem1024.M._poly_getnoise ((Array256.init (fun i_0 => noise2.[512 + i_0])),
    noiseseed, nonce);
    noise2 <- Array1024.init
         (fun i => if 512 <= i < 768 then aux.[i-512]
         else noise2.[i]);
    nonce <- (W8.of_int 7);
    aux <@Jkem1024.M._poly_getnoise ((Array256.init (fun i_0 => noise2.[768 + i_0])),
    noiseseed, nonce);
    noise2 <- Array1024.init
         (fun i => if 768 <= i < 1024 then aux.[i-768]
         else noise2.[i]);
...
proc indcpa_keypair_jazz (pk : W8.t Array1568.t, sk : W8.t Array1536.t, seed : W8.t Array32.t) :
    ...
    aux <@ Aux.inner_product ((Array1024.init (fun i_0 => a.[i_0])),skpv);
    pkpv <- Array1024.init (fun i => if 0 <= i < 256 then aux.[i] else pkpv.[i]);
    aux <@ Aux.inner_product((Array1024.init (fun i_0 => a.[1024 + i_0])),skpv);
    pkpv <- Array1024.init (fun i => if 256 <= i < 512 then aux.[i-256] else pkpv.[i]);
    aux <@ Aux.inner_product ((Array1024.init (fun i_0 => a.[2048 + i_0])), skpv);
    pkpv <- Array1024.init (fun i => if 512 <= i < 768 then aux.[i-512] else pkpv.[i]);
    aux <@ Aux.inner_product ((Array1024.init (fun i_0 => a.[3072 + i_0])), skpv);
    pkpv <- Array1024.init (fun i => if 768 <= i < 1024 then aux.[i-768] else pkpv.[i]);
  ...
  proc iindcpa_enc_jazz (ctp: W8.t Array1568.t, msgp:W8.t Array32.t, pk : W8.t Array1568.t,  noiseseed : W8.t Array32.t) : W8.t Array1568.t = {
    ...
    aux <@Jkem1024.M.__polyvec_pointwise_acc ((Array1024.init (fun i_0 => at.[i_0])),
    sp_0);
    bp <- Array1024.init
          (fun i => if 0 <= i < 256 then aux.[i] else bp.[i]);
    aux <@Jkem1024.M.__polyvec_pointwise_acc ((Array1024.init (fun i_0 => at.[1024 + i_0])),
    sp_0);
    bp <- Array1024.init
          (fun i => if 256 <= i < 512 then aux.[i-256] else bp.[i]);
    aux <@Jkem1024.M.__polyvec_pointwise_acc ((Array1024.init (fun i_0 => at.[2048 + i_0])),
    sp_0);
    bp <- Array1024.init
          (fun i => if 512 <= i < 768
          then aux.[i-512] else bp.[i]);
    aux <@Jkem1024.M.__polyvec_pointwise_acc ((Array1024.init (fun i_0 => at.[3072 + i_0])),
    sp_0);
    bp <- Array1024.init
          (fun i => if 768 <= i < 1024
          then aux.[i-768] else bp.[i]);
...
while(#pre /\ i{1} = i{2} /\ 0 <= i{2} <= 160 /\ j{2} = 2*i{2} /\
       (forall k, 0<=k<2*i{2} =>
...
wp; call (get_noise_sample_noise); auto => /> &1 &2 H0 H1 *; do split. 
+ move : H0; rewrite /lift_array256 /subarray256 setvE /= offunvE 1:/# /= !tP =>H k kb.
  by move : (H k kb);rewrite !mapiE //= !initiE //= 1:/# /= /#. 
+ move : H1; rewrite /lift_array256 /subarray256 setvE /= offunvE 1:/# /= !tP =>H k kb.
  by move : (H k kb);rewrite !mapiE //= !initiE //= !initiE //= /=  /#. 
...
seq 3 4 : (#{/~_N{2}}{~i{2}}pre /\ 
     lift_array256 (subarray256 noise2{1} 0) = (noise2{2}.[0])%Vector /\ _N{2}=4 /\
...
by rewrite !mapiE //= !initiE //= !initiE //= /#. 
 ...
 auto => /> &1 H H0; rewrite /lift_vector /unlift_vector !tP /subarray256 /lift_array256;split;
...
+ by inline  AuxMLKEM.sample_noise2_jasmin; do 2!(unroll for {1} ^while); sim; auto => />.
...
equiv auxienc_good :
 Jkem1024.M.__indcpa_enc ~ AuxMLKEM.iindcpa_enc_jazz :
     ={arg} ==> ={res}. 
proc. 
swap {1} 7 -6.
swap {1} 14 -12.
...
lemma mlkem_correct_kg   : 
   equiv [Jkem1024.M.__indcpa_keypair ~ InnerPKE1024.kg_derand : 
       ...
       let (pk,sk) = res{2} in let (t,rho) = pk in
         sk = Array1536.init (fun i => res{1}.`2.[i]) /\
...
(   randomnessp{1} = coins{2} 
       ...
       let (pk,sk) = r{2} in let (t,rho) = pk in
         sk = Array1536.init (fun i => r{1}.`2.[i]) /\
         t = Array1536.init (fun i => r{1}.`1.[i])  /\
         rho = Array32.init (fun i => r{1}.`1.[i+1536]) ); 1,2: smt().
...
+ move => *;
  ...
  + move => k ??.
    ...
    case ((i{hr} + 192) * 8 <= 1536 + k < (i{hr} + 192) * 8 + 8)=>*; last  by  rewrite /get8 initiE 1:/# /= /#.
     rewrite wordP => i ib.
...
+ auto => /> &1 &2.
  ...
  rewrite /lift_array4096 /lift_polyvec /lift_array256 /invnttv !tP =>  ?val k kbl kbh.
...
+ auto => /> &1 &2.
  ...
  + by rewrite /signed_bound1024_cxq => k kb; rewrite initiE //= /#.
  move => ????result0; rewrite /lift_matrix.
  rewrite /lift_array4096 /lift_polyvec /lift_array256 /invnttv !tP =>  ?val.
  split.
...
wp; ecall{1} (innerprod_corr 
     (invnttv (lift_polyvec (Array1024.init (fun (i_0 : int) => a{1}.[2048 + i_0]))))
     (invnttv (lift_polyvec skpv{1}))).
...
+ auto => /> &1 &2.
  ...
  + by rewrite /signed_bound1024_cxq => k kb; rewrite initiE //= /#.
  move => ????result0; rewrite /lift_matrix.
  rewrite /lift_array4096 /lift_vector /lift_array256 /invnttv !tP =>  ?val.
  do split.
  + move => k klb khb; rewrite !initiE 1:/# /=; do split; 1,2:smt().
...
+ move => r0; rewrite -r0 /=. 
  ...
  rewrite !mapiE /#. 
case(2 = r). 
+ move => r0 r1; rewrite -r0 /kvec  /=.
  rewrite tP => k kb; rewrite !mapiE 1..2:/#  /= !initiE 1,2:/# /=.
  have := H (256*r+k) _;1:smt().
  rewrite !mapiE /#. 
case(1 = r). 
+ move => r0??; rewrite -r0 /kvec /=. 
  rewrite tP => k kb; rewrite !mapiE 1,2:/#  /= !initiE 1,2:/# /=.
  have := H (256*r+k) _;1:smt().
  rewrite !mapiE /#. 
move => *.
...
lemma mlkem_correct_enc _pkp : 
   equiv [Jkem1024.M.__indcpa_enc ~ InnerPKE1024.enc_derand: 
     ...
     pk{1} = _pkp /\
     noiseseed{1} = coins{2} /\
     pk{2}.`1 = Array1536.init( fun i => _pkp.[i]) /\
     pk{2}.`2 = Array32.init( fun i => _pkp.[4*384+i])
       ==> 
     let (c1,c2) = res{2} in
     c1 = Array1408.init(fun i => res{1}.[i]) /\
     c2 = Array160.init(fun i => res{1}.[1408+i]) 
].
proc*.
...
(={msgp,pk,noiseseed,ct} ==> ={r}) 
  (msgp{1} = m{2} /\ 
     ...
     c2 = Array160.init(fun i => r{1}.[1408+i])); 1,2: smt().
...
wp; ecall {1} (polyvec_frombytes_corr 
   (Array1536.init  (fun (i0 : int) => pk0{1}.[0 + i0]))).
+ auto => />  ??rbound;split;last by smt().
  pose xx := (Array1536.init ("_.[_]" _pkp)).
...
while {1} ((tv{2}, rho{2}) = pk0{2} /\ rho{2} = Array32.init (fun (i0 : int) => pk0{1}.[4 * 384 + i0]) /\
           0 <=  w{1} <= 4 /\ inc{1} = 4 /\
...
+ inline AuxMLKEM.__gen_matrix; conseq />.
  ...
  while (i{1} = i{2} /\ 0<=i{1}<=kvec /\ seed{1}=rho{2} /\ trans{1} /\
         forall ii jj, 0<=ii<i{1} => 0<= jj <4 => (a{1}.[ii,jj])%Matrix = (aT{2}.[ii,jj])%Matrix); last
          by auto => /> *; apply: eq_matrixP; smt().
  wp; while (i{1} = i{2} /\ j{1} = j{2} /\ 0<=i{1}<kvec /\ 0<=j{1}<=kvec /\ seed{1}=rho{2} /\ trans{1} /\
         (forall ii jj, 0<=ii<i{1} => 0<= jj <4 => (a{1}.[ii,jj])%Matrix = (aT{2}.[ii,jj])%Matrix) /\
...
seq 20 3 : (u{2} = lift_polyvec bp0{1} /\ v{2} = lift_array256 v{1} /\
            ...
            pos_bound1024_cxq bp0{1} 0 1024 2); last first.
...
+ ecall {1} (polyvec_invntt_corr bp{1}).
  ...
  move => i2 noi1 noi0; move : (r4 (768 + k) _); 1:smt(); rewrite /lift_array1024 mapiE// 1:/# /= i2 /= => ->.
  rewrite /scale mapiE 1:/# /=.
  by rewrite !getvE -ZqField.mulrA (ZqField.mulrC (incoeff 169)) rrinvcoeff ZqField.mulrC ZqField.mul1r.  
...
+ wp; ecall{1}(poly_decompress_corr(Array160.init (fun (i : int) => ct{1}.[4 * 352 + i])));
  by auto => /> /#. 
