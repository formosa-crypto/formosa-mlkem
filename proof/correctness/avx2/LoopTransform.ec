...
proof.
  move => hk /dvdzP [q] ->.
  rewrite (IntOrder.ltr_pmul2r k hk) ltzE -(IntOrder.ler_pmul2r k hk) /#.
qed.

lemma floor_le n k : 0 < k => floor n k <= n.
proof. rewrite {2}(divz_eq n k); smt (modz_cmp). qed.
...
proof.
  ...
  rewrite -(IntOrder.ler_pmul2r k hk); smt (modz_cmp).
qed.

lemma le_floorE (k i n:int) : 0 < k => k %| i => i <= n <=> i <= floor n k.
proof. move => hk kd; smt (divz_eq modz_cmp le_floor). qed.
...
proof.
  ...
  + move=> /> &2 h1 h2 [[]// | h3].
    have h4 := le_floorE (step * k{2}) (i{2} + step * k{2}) n{2} _ _.
