...
equiv a2__absorb_eq:
 M.a2____absorb ~ A2ref.MM.__absorb
 : ={arg} ==> ={res}
 by sim.
...
equiv aBUFLEN__dumpstate_eq:
 M.aBUFLEN____dumpstate ~ ABUFLENref.MM.__dumpstate
 : ={arg} ==> ={res}
 by sim.
...
clone KeccakArrayRef as A1184ref
 with op _ASIZE <- 1184,
      theory A <- Array1184,
      theory WA <- WArray1184
      proof _ASIZE_ge0 by done.

equiv a1184__absorb_eq:
 M.a1184____absorb ~ A1184ref.MM.__absorb
 : ={arg} ==> ={res}
 by sim.
...
clone KeccakArrayRef as A1120ref
 with op _ASIZE <- 1120,
      theory A <- Array1120,
      theory WA <- WArray1120
      proof _ASIZE_ge0 by done.

equiv a1120__absorb_eq:
 M.a1120____absorb ~ A1120ref.MM.__absorb
 : ={arg} ==> ={res}
 by sim.
...
module K = {
  ...
  proc _shake128_absorb_A32_A2 (st:W64.t Array25.t, seed:W8.t Array32.t,
    ...
    var  _1:int;
    st <@ Keccak1600_Jazz.M.__state_init (st);
    (st,  _0) <@ A32ref.MM.__absorb (st, 0, seed, 0, 168);
    (st,  _1) <@ A2ref.MM.__absorb (st, 32, pos, 31, 168);
    return st;
  }
  ...
  W64.t Array25.t * W8.t Array536.t = {
    var offset:int;
    var ms : W64.t;
    st <@ Keccak1600_Jazz.M._keccakf1600 (st);
    ms <- W64.zero;                                                                      
    buf <- SLH64.protect_ptr buf ms;
    offset <- 0;
    (buf, offset) <@ ABUFLENref.MM.__dumpstate (buf, offset, 168, st);
    st <@ Keccak1600_Jazz.M._keccakf1600 (st);
    ms <- W64.zero;                                                                      
    buf <- SLH64.protect_ptr buf ms;
    (buf, offset) <@ ABUFLENref.MM.__dumpstate (buf, offset, 168, st);
    ...
    ms <- W64.zero;                                                                      
    buf <- SLH64.protect_ptr buf ms;
    (buf, offset) <@ ABUFLENref.MM.__dumpstate (buf, offset, 200, st);
    return (st, buf);
  }
  ...
  W8.t Array32.t = {
    ...
    st <- witness;
    st <@ Keccak1600_Jazz.M.__state_init (st);
    (st,  _0) <@ A1184ref.MM.__absorb (st, 0, in_0, 6, 136);
  ...
  W8.t Array32.t = {
    ...
    st <- witness;
    st <@ Keccak1600_Jazz.M.__state_init (st);
    (st,  _0) <@ A1120ref.MM.__absorb (st, 0, in_0, 31, 136);
    ( _1, out) <@ A32ref.MM.__squeeze (st, out, 136);
    return out;
  }
}.
...
wp; ecall (A1184ref.absorb_h [<:W8.t>] in_0 6 136).
...
rewrite /SHA3_256_1184_32 /= tP => i Hi; congr; congr.
rewrite /to_list /SHA3_256 /c512_r8 /KECCAK1600; congr; congr=> //.
rewrite (:1184=1152+32) 1:// mkseq_add 1..2://; congr.
...
proc => /=.
ecall (A32ref.squeeze_h out st 136).
wp; ecall (A1120ref.absorb_h [<:W8.t>] in_0 31 136).
wp; call (state_init_h 136).
auto => /> []/= st1 -> []/= st2 out _ E.
have ->: out = of_list zero (SQUEEZE1600 136 32 (ABSORB1600 (of_int 31) 136 (to_list _in))).
 by rewrite -(Array32.to_listK W8.zero out) /#.
rewrite /SHAKE_256_1120_32 -(Array32.of_listK W8.zero (SHAKE256 _ _)).
...
congr; 1..2:smt().
rewrite (:1120=32+960+128) 1:// !mkseq_add 1..4:/#; congr.
 congr.
  by apply eq_in_mkseq => k Hk /=; rewrite initiE.
 by apply eq_in_mkseq => k Hk /=; rewrite initiE /#.
by apply eq_in_mkseq => k Hk /=; rewrite initiE /#.
...
hoare shake128_absorb_A32_A2_h' _rho _rc:
 ...
 : seed = _rho /\ pos = _rc
 ==>
   absorb_spec_ref 168 31 (to_list _rho ++ to_list _rc) res.
proof.
proc.
ecall (A2ref.absorb_h (to_list _rho) pos 31 168).
wp; ecall (A32ref.absorb_h [<:W8.t>] seed 0 168).
wp; call (state_init_h 168).
...
phoare shake128_absorb_A32_A2_ph _rho _rc:
 [ M._shake128_absorb_A32_A2
   : seed = _rho /\ pos = _rc
     ==>
     res = SHAKE128_ABSORB (to_list _rho ++ to_list _rc)
 ] = 1%r.
proof.
...
hoare shake128_squeeze3blocks_h' _buf _st:
 ...
 : buf=_buf /\ st=_st
 ==> sub res.`2 (0*168) 168 = squeezestate_i 168 _st 0
  /\ sub res.`2 (1*168) 168 = squeezestate_i 168 _st 1
  /\ sub res.`2 (2*168) 168 = squeezestate_i 168 _st 2
  /\ sub res.`2 (2*168) 200 = state2bytes (FIPS202_SHA3_Spec.st_i _st 3).
proof.
proc.
ecall (ABUFLENref.dumpstate_h buf offset 200 st).
...
proc.
call ABUFLENref.dumpstate_ll.
wp;call keccakf1600_ll.
wp;call ABUFLENref.dumpstate_ll.
wp;call keccakf1600_ll.
wp;call ABUFLENref.dumpstate_ll.
wp; call keccakf1600_ll.
by auto.
