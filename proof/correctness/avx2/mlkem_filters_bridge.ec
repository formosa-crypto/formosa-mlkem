...
move => ib.
    ...
    apply W64.init_ext => x xb /=;congr; 1: smt(W64.to_uint_cmp). 
    congr;congr;congr;congr;congr;congr;last by smt(W64.to_uint_cmp). 
    congr; rewrite initiE /=;1: smt(W64.to_uint_cmp). 
    by rewrite initiE /=; smt(W64.to_uint_cmp).
qed.
...
proof. 
    ...
    rewrite initiE 1:/# /=.
    rewrite ifF;1:smt().
    rewrite get_to_uint; congr;  1:smt().
    rewrite bits8_div 1:/# /=.
    congr;congr. 
    have -> : (2 * i + k %/ 8) %/ 2 = i by smt().
    have -> : 8 * ((2 * i + k %/ 8) %% 2) = if 8 <= k then 8 else 0 by smt().
    case (8 <= k) => /= *.
    + rewrite (modz_small _ 256);1: smt(W16.to_uint_cmp pow2_16).
