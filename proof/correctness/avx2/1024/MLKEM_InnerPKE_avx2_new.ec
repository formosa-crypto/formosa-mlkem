...
require import MLKEM_Poly_avx2_circuit MLKEM_Poly_avx2_circuit_1024 MLKEM_PolyVec_avx2_circuit.
...
by rewrite /A3 Hva /lift_polyvec KVec.initiE; smt(kvec_val).
...
by rewrite /B3 Hvb /lift_polyvec KVec.initiE; smt(kvec_val).
...
rewrite (Big.BAdd.big_int_recl 3) // (Big.BAdd.big_int_recl 2) // (Big.BAdd.big_int_recl 1) // (Big.BAdd.big_int_recl 0) // Big.BAdd.big_geq //=.
rewrite !add_comm_ntt !nttK.
rewrite !poly2algE.
smt(kvec_val). smt(kvec_val). smt(kvec_val). smt(kvec_val). smt(kvec_val). smt(kvec_val). smt(kvec_val). smt(kvec_val).
rewrite /R /P0 /P1 /P2 /P3 /A0 /A1 /A2 /A3 /B0 /B1 /B2 /B3.
...
lemma encode12_vec_tobytes_bridge (X : W16.t Array1024.t) :
  encode12_vec (toipolyvec (lift_polyvec X))
  = BytesPKVec.init (fun i =>
      (Array1536.of_list W8.zero
         (ByteEncode 12 (to_list (map asint (lift_array1024 X))))).[i]).
proof.
have Hk1024 : 256 * Parameters.kvec = 1024 by smt(kvec_val).
have H1536 : 384 * Parameters.kvec = 1536 by smt(kvec_val).
rewrite /encode12_vec toipolivec_lift.
have ->: to_list (IPVec.init (fun (idx : int) => asint (lift_array1024 X).[idx]))
       = to_list (map asint (lift_array1024 X)).
+ apply (eq_from_nth witness).
  + rewrite IPVec.size_to_list Array1024.size_to_list; smt().
  move => i; rewrite IPVec.size_to_list => Hi.
  have Hi1024 : 0 <= i < 1024 by rewrite -Hk1024; exact Hi.
  rewrite IPVec.get_to_list Array1024.get_to_list IPVec.initiE 1:/#.
  simplify.
  rewrite Array1024.mapiE 1:/#.
  by rewrite /lift_array1024 Array1024.mapiE 1:/# Array1024.mapiE 1:/#.
apply BytesPKVec.tP => k Hk.
have Hk1536 : 0 <= k < 1536 by rewrite -H1536; exact Hk.
by rewrite get_of_list 1:/# initiE 1:/# /= get_of_list 1:/#.
qed.

lemma lift_array1024_unlift_ofipolyvec (vi : ipolyvec) :
  all (fun i => 0 <= i < 32768) vi =>
  lift_array1024 (unlift_polyvec (ofipolyvec vi)) = Array1024.init (fun k => incoeff vi.[k]).
proof.
rewrite allP => Hbnd.
have kv := kvec_val.
rewrite /lift_array1024 /unlift_polyvec /ofipolyvec tP => k kb.
rewrite Array1024.mapiE 1:/# /= Array1024.initiE 1:/# /= KVec.initiE 1:/# /= mapiE 1:/# /= /subarray256 initiE 1:/# /=.
...
lemma lift_polyvec_subarray1024_unlift_matrix (a : polymat) (i j : int) :
  0 <= i < 4 => 0 <= j < 4 =>
  (lift_polyvec (subarray1024 (unlift_matrix a) i)).[j] = a.[(i, j)].
proof.
move => Hi Hj.
have kv := kvec_val.
...
by rewrite (lift_polyvec_subarray1024_unlift_matrix a i j _ _); smt(kvec_val).
qed.
...
lemma decode_vec_c1_bridge (f : int -> W8.t) :
  decode_vec (BytesCtVec.init f) = Circuit_sem_1024.decode11_vec (Array1408.init f).
proof.
have Hdu : Parameters.du = 11 by smt(Parameters.param_sets kvec_val).
have Hsz : 32 * Parameters.du * Parameters.kvec = 1408 by smt(Parameters.param_sets kvec_val).
rewrite /decode_vec Hdu; congr; congr.
apply (eq_from_nth witness).
+ rewrite BytesCtVec.size_to_list Array1408.size_to_list; smt().
move => i; rewrite BytesCtVec.size_to_list => Hi.
have Hi1408 : 0 <= i < 1408 by smt().
by rewrite BytesCtVec.get_to_list Array1408.get_to_list initiE 1:/# initiE 1:/#.
qed.

lemma decode_poly_c2_bridge (f : int -> W8.t) :
  decode_poly (BytesPoly.init f) = Circuit_sem_1024.decode5 (Array160.init f).
proof.
have Hdv : Parameters.dv = 5 by smt(Parameters.param_sets kvec_val).
have Hsz : 32 * Parameters.dv = 160 by smt(Parameters.param_sets kvec_val).
rewrite /decode_poly Hdv; congr; congr.
apply (eq_from_nth witness).
+ rewrite BytesPoly.size_to_list Array160.size_to_list; smt().
move => i; rewrite BytesPoly.size_to_list => Hi.
have Hi160 : 0 <= i < 160 by smt().
by rewrite BytesPoly.get_to_list Array160.get_to_list initiE 1:/# initiE 1:/#.
qed.


lemma frombytes12_decode_bridge (f : int -> W8.t) :
  Array1024.of_list (incoeff 0)
    (map incoeff (ByteDecode 12 (to_list (Array1536.init f))))
  = Array1024.init (fun kk => incoeff (decode12_vec (BytesPKVec.init f)).[kk]).
proof.
have Hd : size (ByteDecode 12 (to_list (Array1536.init f))) = 1024.
+ rewrite (size_decode 4 12 (to_list (Array1536.init f))) //; smt(Array1536.size_to_list).
have Hbytes : to_list (Array1536.init f) = to_list (BytesPKVec.init f).
+ apply (eq_from_nth witness); 1: (rewrite Array1536.size_to_list BytesPKVec.size_to_list; smt(kvec_val)).
  move => i; rewrite Array1536.size_to_list => Hi.
  rewrite Array1536.get_to_list BytesPKVec.get_to_list.
  rewrite Array1536.initiE //= BytesPKVec.initiE //=.
  smt(kvec_val).
apply Array1024.tP => k Hk.
rewrite get_of_list 1:/# (nth_map 0).
+ by rewrite Hd.
rewrite Array1024.initiE //= /decode12_vec get_of_list.
+ smt(kvec_val).
by rewrite Hbytes.
qed.
...
lemma mlkem_correct_enc_avx2_op _pkp :
  equiv [Jkem_avx2.M.__indcpa_enc ~ InnerPKE_Op.enc_derand :
    ...
    c2 = BytesPoly.init (fun i => res{1}.[i+1408])].
proof.
proc => /=.
...
seq 14 9 : (#pre /\
            publicseed{1} = rho{2} /\
            lift_array1024 pkpv{1} = nttunpackv (Array1024.init (fun kk => incoeff (decode12_vec pk{2}.`1).[kk])) /\
...
+ sp 1 0.
  ...
  + (* lift_polyvec sp_0 = rv{2} *)
    have Hrv : forall i, 0 <= i < Parameters.kvec => rv{2}.[i] = samplePolyCBD (PRF r{2} (W8.of_int i)).
    + move => i Hi.
      have ->: rv{2}.[i] = (prg_enc_inner r{2}).`1.[i] by rewrite Hrv_eq.
      by rewrite /prg_enc_inner /= KVec.initiE 1:/# /=.
    apply KVec.tP => i Hi.
    rewrite Hrv 1:/# /lift_polyvec KVec.initiE 1:/# /=.
    have Hspi := Hsp_fa i _; first by smt(kvec_val).
    rewrite tP => k Hk.
  ...
  + (* lift_polyvec ep = e1{2} *)
    ...
    + rewrite Ci0 /= initiE 1:/# /= initiE 1:/# /= initiE 1:/# /= initiE 1:/# /= ifF 1:/# ifF 1:/# ifF 1:/# ifT 1:/#.
      ...
      by congr; rewrite Hep_eq /prg_enc_inner /= KVec.initiE 1:/# /= /PRF kvec_val /=.
    case (i = 1) => Ci1.
...
+ sp 1 0.
  ...
  auto => |> &1 &2;  rewrite /prg_enc_inner=>  ??????[#]?????? result Hepp_eq Hepp_bnd;split;smt(kvec_val).
       ...
       + move => &m z.
         ...
         + move => ???????? rr Hrb Hrv;do split;1,2,5..:smt().
           + move => j jbl jbh.
             ...
             rewrite mapiE 1:/# /= Array256.initiE 1:/# /= Array1024.initiE 1:/# /=.
             rewrite ifT 1:/#.
...
+ ecall{1} (polyvec_invntt_avx2_corr (nttpackv bp{1})).
  ...
  + have <- : (lift_polyvec result).[j]%Vector = lift_array256 (subarray256 result j)%Serialization.
       by rewrite /lift_polyvec KVec.initiE /=;1:smt(kvec_val).
...
+ have H := polyvec_add_corr_avx 1 1 _ _ => //.
  ...
  apply KVec.tP => j Hj.
  have Hj3 : 0 <= j < 4 by smt(kvec_val).
  ...
  have Hjk : 0 <= 256 * j + k < 1024 by smt(kvec_val).
  rewrite Hresk 1:/#.
  by rewrite /lift_array1024 mapiE 1:/# mapiE 1:/#.
...
auto =>  |>  &1 &2 ?? Hpkv Hpkb Hkv Hkb; rewrite /prg_enc_inner => [#] Hrv Hep Hepp Hmb Hepb Heppb Hrvv Hspb Hbpv Hvv Hvb Hbpb result Hresv result0 Hres0.
have Hbnd : 32 * Parameters.du * Parameters.kvec = 1408 by smt(Parameters.param_sets kvec_val).
have Hdu : Parameters.du = 11 by smt(Parameters.param_sets kvec_val).
have Hdv : Parameters.dv = 5 by smt(Parameters.param_sets kvec_val).
split.
+ rewrite tP => k kn.
...
+ sp 1 0.
  ...
  have Hs2v : forall i, 0 <= i < Parameters.kvec => (prg_kg_inner coins{2}).`2.[i] = samplePolyCBD (PRF (G_coins_ds coins{2}).`2 (W8.of_int i)).
  + by move => i Hi; rewrite /prg_kg_inner /= KVec.initiE 1:/# /=.
  have He3v : forall i, 0 <= i < Parameters.kvec => (prg_kg_inner coins{2}).`3.[i] = samplePolyCBD (PRF (G_coins_ds coins{2}).`2 (W8.of_int (i + Parameters.kvec))).
  + by move => i Hi; rewrite /prg_kg_inner /= KVec.initiE 1:/# /=.
  do split.
  + (* lift_polyvec skpv = (prg_kg_inner coins).`2 *)
    apply KVec.tP => i Hi.
    rewrite Hs2v 1:/# /lift_polyvec KVec.initiE 1:/# /=.
    have Hspi := Hskpv i _; first by smt(kvec_val).
    rewrite tP => k Hk.
    move: Hspi; rewrite /lift_array256 /subarray256 tP => /(_ k _); first by smt().
    by rewrite !mapiE 1:/# /=.
  + (* lift_polyvec e = (prg_kg_inner coins).`3 *)
    apply KVec.tP => i Hi.
    rewrite He3v 1:/# /PRF /lift_polyvec KVec.initiE 1:/# /=.
    rewrite /lift_array256 /subarray256 tP => k Hk.
    rewrite mapiE 1:/# /= initiE 1:/# /=.
    case (i = 0) => Ci0.
    + rewrite Ci0 /= initiE 1:/# /= initiE 1:/# /= initiE 1:/# /= initiE 1:/# /= ifF 1:/# ifF 1:/# ifF 1:/# ifT 1:/#.
      have := Hr1; rewrite /lift_array256 tP => /(_ k _); 1: smt(); rewrite mapiE 1:/# /= => ->.
      by congr; rewrite kvec_val /#.
    case (i = 1) => Ci1.
    + rewrite Ci1 /= initiE 1:/# /= initiE 1:/# /= initiE 1:/# /= initiE 1:/# /= ifF 1:/# ifF 1:/# ifT 1:/#.
      have := Hr2; rewrite /lift_array256 tP => /(_ k _); 1: smt(); rewrite mapiE 1:/# /= => ->.
      by congr; rewrite kvec_val /#.
...
seq 2 2 : (#{/~s{2} = (prg_kg_inner coins{2}).`2}
            ...
            {/~forall i, 0 <= i < 4 =>
                 lift_array256 (Array256.init (fun k => skpv{1}.[256*i + k])) =
                 samplePolyCBD (Symmetric.PRF noiseseed{1} (W8.of_int i))}pre /\
...
lemma mlkem_correct_dec :
  equiv [Jkem_avx2.M.__indcpa_dec ~ KPKE.decrypt :
    ...
    let (c1,c2) = cph{2} in
      ...
      c2 = BytesPoly.init (fun i => ct{1}.[1408+i])
    ==>
    res{1} = res{2}].
proof.
proc => /=.
...
+ wp; ecall{1}(polyvec_decompress_corr (Array1408.init (fun (i : int) => ct{1}.[0 + i]))).
  ...
  move: Hb; rewrite /pos_bound1024_cxq /signed_bound1024_cxq /bpos16 /b16 /#.
...
seq 1 3 : (#pre /\ lift_array1024 skpv{1} = nttunpackv (Array1024.init (fun kk => incoeff (decode12_vec sk{2}).[kk])) /\
...
+ wp; ecall{1}(polyvec_frombytes_corr sk{1}).
  ...
  rewrite Hr; congr.
  have ->: to_list sk{1} = to_list (Array1536.init (fun i => sk{1}.[i])).
  + by congr; apply Array1536.tP => k Hk; rewrite initiE 1:/#.
