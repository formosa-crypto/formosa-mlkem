...
from JazzEC require import Jkem768_avx2_stack.
...
from CryptoSpecs require import MLKEM768 Symmetric.
import Symmetric768.
...
lemma copy1088 (a : W8.t Array1088.t) :
Array1088.init (fun (i : int)  =>  WArray1088.get8 (WArray1088.init64                                                                       
               (fun (i0 : int) =>  copy_64 (Array136.init                                                                             
               (fun (i1 : int) =>  WArray1088.get64 (WArray1088.init8                                                                  
               (fun (i2 : int) =>  a.[i2]))  i1)).[i0])) i) = a.  
rewrite tP => k kb.
rewrite initiE 1:/# /= /get8.
rewrite initiE 1:/# /= /copy_64.
...
lemma copy1184 (a : W8.t Array1184.t) :
Array1184.init (fun (i : int)  =>  WArray1184.get8 (WArray1184.init64                                                                       
               (fun (i0 : int) =>  copy_64 (Array148.init                                                                             
               (fun (i1 : int) =>  WArray1184.get64 (WArray1184.init8                                                                  
               (fun (i2 : int) =>  a.[i2]))  i1)).[i0])) i) = a.  
rewrite tP => k kb.
rewrite initiE 1:/# /= /get8.
rewrite initiE 1:/# /= /copy_64.
...
lemma copy2400 (a : W8.t Array2400.t) :
Array2400.init (fun (i : int)  =>  WArray2400.get8 (WArray2400.init64                                                                       
               (fun (i0 : int) =>  copy_64 (Array300.init                                                                             
               (fun (i1 : int) =>  WArray2400.get64 (WArray2400.init8                                                                  
               (fun (i2 : int) =>  a.[i2]))  i1)).[i0])) i) = a.                                                               
rewrite tP => k kb.
rewrite initiE 1:/# /= /get8.
rewrite initiE 1:/# /= /copy_64.
...
proc rewrite {1} 16 (copy1184).
proc rewrite {1} 17 (copy2400).
sp;wp;inline {1} 1.
...
seq 11 1 : (
      ...
      pk{2}.`2 = Array32.init (fun i => sk0{1}.[i + 1152 + 1152]) /\
      H_pk pk{2} = Array32.init (fun i => sk0{1}.[i + 1152 + 1152 + 32]) /\
...
+ while {1} (inc{1} = 4 /\
      ...
      pk{2}.`2 = Array32.init (fun i => pk0{1}.[i + 1152]) /\
       0 <= i{1} <= 4 /\ 
       forall k, 0<=k<i{1} => 
          WArray2400.get64_direct (WArray2400.init 
            (fun ii => sk0{1}.[ii])) (2368 + k*8) =  
...
seq 2 1 : (
      ...
      pk{2}.`1 = Array1152.init (fun i => pk0{1}.[i]) /\
      pk{2}.`2 = Array32.init (fun i => pk0{1}.[i + 1152])). 
call (mlkem_correct_kg_avx2_stack).
auto => /> /#. 
...
+ wp;while {1} (#pre /\ 
  ...
  +  move => i ib; rewrite initiE 1:/# /= initiE 1:/# /= initiE 1:/# /=.
      rewrite get8_set64_directE 1,2:/# /=.
      case  ((144 + i{!hr}) * 8 <= i && i < (144 + i{!hr}) * 8 + 8) => *.
      + by rewrite WArray1184.get64E pack8bE 1:/# initiE 1:/# /= initiE /#.
      by rewrite /get8 initiE; smt(Array32.initiE).
  + by smt().
  + by smt().
  +  move => i ib ibb; rewrite initiE 1:/# /=.
      rewrite get8_set64_directE 1,2:/# /=.
      case ((144 + i{!hr}) * 8 <= 1152 + i && 1152 + i < (144 + i{!hr}) * 8 + 8) => *.
      + rewrite WArray1184.get64E pack8bE 1:/# initiE 1:/# /=.
      by rewrite /get8 initiE; smt(Array1152.initiE).
    + by rewrite /get8 initiE; smt(Array1152.initiE).
  +  move => i ib ibb; rewrite initiE 1:/# /=.
      rewrite get8_set64_directE 1,2:/# /=.
      case ( (144 + i{!hr}) * 8 <= 1152 + i && 1152 + i < (144 + i{!hr}) * 8 + 8) => *.
...
ecall {1} (sha3_256A_M1184_ph_stack pk0{1}); 1: by inline *; auto => />* /#.  

wp;while {1} (#pre /\ 
         0 <= i{1} <= 4 /\ 
         forall k, 0 <= k < i{1} * 8 => sk0{1}.[2336 + k] = ((H_pk pk{2})).[k]) (4 - i{1}).
move => &m z; auto => /> &1 &2; rewrite  !tP.
move => pkv1s pkv2s pkv1 pkv2 ? prev ?;do split.
  +  move => i ib; rewrite initiE 1:/# /= initiE 1:/# /= initiE 1:/# /=.
      rewrite get8_set64_directE 1,2:/# /=.
      case  ((292 + i{!1}) * 8 <= i && i < (292 + i{!1}) * 8 + 8) => *.
      + by rewrite WArray32.get64E pack8bE 1:/# initiE 1:/# /= initiE /#.
      by rewrite /get8 initiE; smt(Array2400.initiE).
  +  move => i ib; rewrite initiE 1:/# /= initiE 1:/# /= initiE 1:/# /=.
      rewrite get8_set64_directE 1,2:/# /=.
      case  ((292 + i{!1}) * 8 <= i && i < (292 + i{!1}) * 8 + 8) => *.
      + by rewrite WArray32.get64E pack8bE 1:/# initiE 1:/# /= initiE /#.
  ...
  +  move => i ib; rewrite initiE 1:/# /= initiE 1:/# /=.
      rewrite get8_set64_directE 1,2:/# /=.
      case  ((292 + i{!1}) * 8 <= i + 1152 && i + 1152 < (292 + i{!1}) * 8 + 8) => *.
      + by rewrite WArray32.get64E pack8bE 1:/# initiE 1:/# /= initiE /#.
      by rewrite /get8 initiE; smt(Array1152.initiE).
  +  move => i ib; rewrite initiE 1:/# /= initiE 1:/# /=.
      rewrite get8_set64_directE 1,2:/# /=.
      case  ((292 + i{!1}) * 8 <= i + 2304 && i + 2304 < (292 + i{!1}) * 8 + 8) => *.
      + by rewrite WArray32.get64E pack8bE 1:/# initiE 1:/# /= initiE /#.
      by rewrite /get8 initiE; smt(Array32.initiE).
  + by smt().
  + by smt().
  +  move => i ib ibb; 
      rewrite initiE 1:/# /=.
      rewrite get8_set64_directE 1,2:/# /=.
      case ( (292 + i{!1}) * 8 <= 2336 + i && 2336 + i < (292 + i{!1}) * 8 + 8) => *.
      + rewrite WArray32.get64E pack8bE 1:/# initiE 1:/#.
      rewrite /get8 initiE 1:/# /= initiE 1:/# /=.
      by rewrite /H_pk /SHA3_256_1184_32 /= get_of_list /#.
    + by rewrite /get8 initiE; smt(Array1152.initiE).
  + by smt().
...
proc => //=.
proc rewrite {1} 10 (copy1184).
proc rewrite {1} 11 (copy32).
proc rewrite {1} 21 (copy1088).
...
+ wp;conseq />. while {1} (
  ...
  + move => *; smt(Array960.initiE).
  + move => *; do split;1:smt().
  by move => *; smt(Array32.tP).

wp; call (mlkem_correct_enc_avx2_stack).
wp; ecall {1} (sha3_512A_512A_A64_stack coins{2} (H_pk pk{2})).
...
lemma verify_correct_h_stack _ctp _ctp1 :
  hoare [Jkem768_avx2_stack.M.__verify : 
             arg = (_ctp,_ctp1) ==>
             (_ctp = _ctp1 => 
                       res = W64.of_int 0) /\
             (_ctp <> _ctp1 => 
...
auto => /> &hr ?? [HL HR] ?.
pose x := 
      ((WArray1088.get256 ((WArray1088.init8 ("_.[_]" _ctp1))) i{hr})) `^`
      (WArray1088.get256 ((WArray1088.init8 ("_.[_]" _ctp))) i{hr}).
do split; 1..2: by smt().
+ move => H0 k kbl kbh.
...
lemma verify_ll : islossless Jkem768_avx2_stack.M.__verify.
proc.
wp.
...
lemma verify_correct_stack _ctp _ctp1 :
  phoare  [Jkem768_avx2_stack.M.__verify : 
             arg = (_ctp,_ctp1) ==>
             (_ctp = _ctp1 => 
                       res = W64.of_int 0) /\
             (_ctp <> _ctp1 => 
                       res = W64.of_int 1)] = 1%r 
   by conseq verify_ll (verify_correct_h_stack _ctp _ctp1).

lemma cmov_correct_h_stack _dst _src   _cnd  :
   hoare [Jkem768_avx2_stack.M.__cmov : 
...
lemma cmov_ll_stack : islossless Jkem768_avx2_stack.M.__cmov by islossless.


lemma cmov_correct_stack _dst _src _cnd:
   phoare [Jkem768_avx2_stack.M.__cmov : 
        dst=_dst /\   src = _src /\ cnd = _cnd  ==>
             (_cnd = W64.of_int 0 => 
               res = _src) /\
             (_cnd = W64.of_int 1 => 
...
proc. 
proc rewrite {1} 8 (copy1088).
proc rewrite {1} 9 (copy2400).
proc rewrite {1} 17 (copy32).
inline {1} 13.
proc rewrite {1} 25 (copy2400).

swap {1} [16..22] -15.
seq 13 0 : #pre; 1: by auto.
sp;seq 1 1 : (#pre /\ aux{1} = m{2}); 
  1: by call (mlkem_correct_dec_avx2_stack); 1: by auto => /> /#.

seq 5 1 : (#pre /\ 
           (forall k, 0<=k<32 => buf{1}.[k] = m{2}.[k]) /\
           (forall k, 0<=k<32 => kr{1}.[k] = _K{2}.[k]) /\
           (forall k, 0<=k<32 => kr{1}.[k+32] = r{2}.[k])).
+ ecall {1} (sha3_512A_512A_A64_stack m{2} hpk{2}).
  wp; conseq (_: _ ==> 
   (forall k, 0<=k<32 => buf{1}.[k] = m{2}.[k]) /\
   (forall k, 32<=k<64 => buf{1}.[k] = s_sk{1}.[2336 + k - 32]) /\
...
+ auto => /> &1 &2; rewrite  !tP => ???buf bvl bvh ; split.
  ...
  + auto => /> &1 &2 ??? /=;do split.
    ...
    + by move => *;rewrite initiE 1:/# /= ifT 1:/#;smt(Array1152.initiE).
    + by smt().
  move => *; auto => /> &hr ?????; do split; 1..2,5: by smt().
  +  move => k kbl kbh; rewrite initiE 1:/# /=.
      rewrite get8_set64_directE 1,2:/# /=.
      case  ((i{hr} + 4) * 8 <= k && k < (i{hr} + 4) * 8 + 8 ) => *.
      + by rewrite WArray2400.get64E pack8bE 1:/# initiE 1:/# /= initiE /#.
      by rewrite /get8 initiE; smt(Array1152.initiE).
  +  move => k kbl kbh; rewrite initiE 1:/# /=.
      rewrite get8_set64_directE 1,2:/# /=.
      case  ((i{hr} + 4) * 8 <= k && k < (i{hr} + 4) * 8 + 8) => *.
...
+ call (mlkem_correct_enc_avx2_stack).

  auto => /> &1 &2 ?; rewrite !tP => ?????; do split.
  + by smt(Array32.initiE).
  + by smt(Array32.initiE).
  + move => *;rewrite initiE 1:/# /= initiE 1:/# /=.
    by smt(Array1152.initiE).
  + move => *;rewrite initiE 1:/# /= initiE 1:/# /=.
    by smt(Array32.initiE).
  ...
  rewrite tP => i ib.
  rewrite !initiE //=. 
  by  smt(Array128.initiE Array960.initiE).
...
+  conseq (_: _ ==> (c{2}  = cph{2} => cnd{1} = W64.of_int 0) /\
   ...
   ecall {1} (verify_correct_stack ct0{1} ctc{1}).
   auto => /> &1 &2; rewrite  /= !tP.
...
wp;ecall {1} (cmov_correct_stack shk0{1} (Array32.init (fun (i_0 : int) => kr{1}.[0 + i_0])) cnd{1}).
...
+ wp;while {1} (#pre /\ 
  ...
  +  move => k kb; rewrite initiE 1:/# /= initiE 1:/# /= initiE 1:/# /=.
      rewrite get8_set64_directE 1,2:/# /=.
      case  ((i{hr} + 4) * 8 <= k && k < (i{hr} + 4) * 8 + 8) => *.
      + by rewrite WArray1088.get64E pack8bE 1:/# initiE 1:/# /= initiE /#.
      by rewrite /get8 initiE; smt(Array32.initiE).
  + by smt().
  + by smt().
  ...
  +  move => k kb kbb; rewrite initiE 1:/# /=.
      ...
      case ((i{hr} + 4) * 8 <= 32 + k && 32 + k < (i{hr} + 4) * 8 + 8) => *.
      + rewrite WArray1088.get64E pack8bE 1:/# initiE 1:/# /=.
      by rewrite /get8 initiE 1:/#;  smt(Array128.initiE).
    + rewrite /get8 initiE 1:/# /=;  smt(Array960.initiE).
  + by smt().
  ...
  + move => *; rewrite tP => kk kkb;rewrite initiE 1:/# /=.
    ...
    + congr;rewrite tP => jj jjb;smt(Array960.initiE).
    + rewrite tP => jj jjb;smt(Array128.initiE).
