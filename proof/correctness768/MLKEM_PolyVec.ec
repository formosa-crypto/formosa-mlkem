...
wp -4.
simplify.
conseq (: a = _aw /\ Array768.all (fun c => c \ult W16.of_int (2*3329)) a ==> _).
+ move => &hr />; rewrite allP /= /pos_bound768_cxq /(\ult) /= /qE.
...
conseq (: _
 ==>
 rp =
 let ret = init_768_12 (fun j => tobytes_circuit _aw.[j]) in
  init_1152_8 (fun i =>
    W8.init (fun j =>
      let idx = i*8 + j in
      let aidx = idx %/ 12 in
