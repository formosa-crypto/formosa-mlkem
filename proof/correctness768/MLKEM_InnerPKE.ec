...
proc sample_noise2_jasmin(noiseseed:W8.t Array32.t) : W16.t Array768.t * W16.t Array768.t = {
    ...
    noise2 <- witness;
    nonce <- (W8.of_int 0);
    aux <@Jkem768.M._poly_getnoise ((Array256.init (fun i_0 => noise1.[0 + i_0])),
    noiseseed, nonce);
    ...
    nonce <- (W8.of_int 3);
    aux <@Jkem768.M._poly_getnoise ((Array256.init (fun i_0 => noise2.[0 + i_0])),
    noiseseed, nonce);
...
proc indcpa_keypair_jazz (pkp:W64.t, skp:W64.t, seed:W8.t Array32.t) : unit = {
   ...
   Jkem768.M.__polyvec_tobytes (skp, skpv);
   Jkem768.M.__polyvec_tobytes (pkp, pkpv);
  ...
  proc redcompr(ctp:W64.t,bp:W16.t Array768.t) : unit = {
    ...
    Jkem768.M.__polyvec_compress (ctp, bp);
  }
  ...
  proc indcpa_enc_jazz (ctp:W64.t, msgp:W8.t Array32.t, pkp:W64.t, noiseseed : W8.t Array32.t) : unit = {
    ...
    pkpv <- witness;
    pkpv <@Jkem768.M.__polyvec_frombytes (pkp);
    ...
    aux <@Jkem768.M.__polyvec_pointwise_acc ((Array768.init (fun i_0 => at.[0 + i_0])),
    sp_0);
    bp <- Array768.init
          (fun i => if 0 <= i < 0 + 256 then aux.[i-0] else bp.[i]);
    aux <@Jkem768.M.__polyvec_pointwise_acc ((Array768.init (fun i_0 => at.[3*256 + i_0])),
    sp_0);
    bp <- Array768.init
    ...
    v <@Jkem768.M._poly_compress (ctp, v);
...
equiv squeezeblock_ignore :
 Jkem768.M._shake128_squeezeblock ~Jkem768.M._shake128_squeezeblock :
   arg{1}.`1 = arg{2}.`1 ==> ={res}.
proof.
...
(* 
equiv absorb_ignore :
 Jkem768.M._shake128_absorb34 ~Jkem768.M._shake128_absorb34 :
   arg{1}.`2 = arg{2}.`2 ==> ={res}.
proof.
...
equiv shake33_ignore :
 Jkem768.M._shake256_33_128 ~Jkem768.M._shake256_33_128 :
   arg{1}.`2 = arg{2}.`2 ==> ={res}.
proof.
proc=> /=. 
...
equiv auxenc_good :
 Jkem768.M.__indcpa_enc ~ AuxMLKEM.indcpa_enc_jazz :
...
equiv auxienc_good :
 Jkem768.M.__iindcpa_enc ~ AuxMLKEM.iindcpa_enc_jazz :
     ={Glob.mem,arg} ==> ={Glob.mem,res}. 
proc. 
swap {1} 6 -5.
swap {1} 14 -12.
...
+ move => *;
  ...
  swap {2} 2 -1.
  wp; ecall {1} (polyvec_tobytes_corr Glob.mem{1} (to_uint pkp{1}) (lift_array768 pkpv{1})). 
  wp; ecall {1} (polyvec_tobytes_corr Glob.mem{1} (to_uint skp{1}) (lift_array768 skpv{1})).
  auto => /> &1 &2.
  ...
  + rewrite /load_array1152 tP => k kb; rewrite !initiE //=. 
    ...
    + move : Hl1;rewrite /load_array1152 /encode12_vec tP => Hl.
      move : (Hl k _); 1:smt().
      rewrite initiE 1:/# /= get_of_list 1:/# /= => ->. 
      by do congr; apply toipolivec_lift.
    + move : Hl2;rewrite /load_array1152 /encode12_vec !tP => Hl k kb.
      move : (Hl k _); 1:smt().
...
wp; ecall {1} (polyvec_compress_corr Glob.mem{1} (to_uint ctp{1})  (lift_array768 bp0{1}) ).
auto => /> &1 &2; rewrite /signed_bound_cxq /signed_bound768_cxq.
move => ???;split;1:smt().
move => ?? mem1; rewrite  /touches /load_array960 /compress_polyvec /load_array128 /lift_array768 /fromarray256.
move => t1 rr1; do split;rewrite to_uintD_small /=;1..5: smt(W64.to_uint_cmp).
move => ?? mem2 t2 rr2;do split.
+ move => i ib;rewrite -t1 1:/#. 
  have ->: to_uint ctp{1} + i = to_uint ctp{1} + 960 + (i - 960) by ring.
  by rewrite t2 /#.
+ have -> : Array960.init (fun (i0 : int) => mem2.[to_uint ctp{1} + i0])
       = Array960.init (fun (i0 : int) => mem1.[to_uint ctp{1} + i0]).
...
lemma mlkem_correct_ienc mem _pkp : 
   equiv [Jkem768.M.__iindcpa_enc ~ InnerPKE768.enc_derand: 
     valid_ptr _pkp (384*3 + 32) /\
     Glob.mem{1} = mem /\ 
     msgp{1} = m{2} /\ 
     to_uint pkp{1} = _pkp /\
...
ecall{1}(poly_tomsg_corr (lift_array256 mp{1})); auto => /> /#.
qed.
