...
proof.
elim: x => //= x xs IH /=.
rewrite !val_digits_cons IH -pow_add //.
 by apply size_ge0.
by rewrite pow1; ring.
qed.
...
(* axiomatized by mul1_digitsE.*)

(*hint simplify mul1_digitsE.*)

lemma mul1_digitsCE x y:
 mul1_digits x y = List.map (transpose Int.( * ) x) y.
proof. by elim: y => //= y ys IH; split => //=; ring. qed.
...
proof.
elim: x => // x xs IH.
by rewrite /= add_digitsP mul1_digitsP !val_digits_cons IH; ring.
qed.
...
lemma ubW64M nx ny (x y: W64.t):
 ...
 ubW64 ny y =>
 ubW64 (nx*ny) (x*y).
proof. rewrite /ubW64 to_uintM; smt (le_modz W64.to_uint_cmp). qed.
...
proof.
move=> /bW64P [? ?] /bW64P [? ?]; rewrite bW64ub 1:/#.
have T := (ubW64D _ _ _ _ H0 H2).
apply (ubW64W _ _ _ _ T).
apply (lez_trans (2^(max nx ny + 1)-1)).
 rewrite powS 1:/#.
 case: (nx <= ny) => ?.
  apply (ler_trans (2 ^ ny - 1 + (2 ^ ny - 1))); first smt(pow_Mle).
  smt (max_is_ub).
 smt(pow_Mle).
smt(pow_Mle).
qed.
...
proof.
rewrite bW64E /mask => /> *.
rewrite to_uint_and_mod //.
have HH: 0 < 2^n by apply powPos.
by move: (modz_cmp (to_uint x) (2^n) HH); progress.
qed.
...
rewrite to_uint_and_mod // modz_small //.
...
case: (c < W64.modulus) => *.
 ...
 smt().
rewrite H /=.
have [? ?]:= modz_cmp c W64.modulus _; first by apply W64.gt0_modulus.
...
rewrite pow_add // Ring.IntID.subrK => *.
have ? := pow_Mle n 64 _; first by smt().
rewrite modz_small //.
apply bound_abs; split => *; first smt(divr_ge0 W64.to_uint_cmp gt0_pow2).
by apply (ltr_le_trans _ _ _ H4); apply pow_Mle; smt().
qed.
...
proof.
rewrite !bW64E !andabP=> /> *; apply W64.to_uintM_small.
...
lemma val_limbs_ge0 base x:
 0 < base => 0 <= val_limbs base x.
proof.
move=> Hbase; elim: x => //= x xs IH.
by rewrite val_digits_cons; smt(W64.to_uint_cmp).
qed.

lemma val_limbs_cons_small base x xs y:
 0 < base =>
 y < base =>
 ((val_limbs base (x::xs)) <= y) = (to_uint x <= y /\ val_limbs base xs = 0).
proof.
move=> * /=; rewrite val_digits_cons.
smt(W64.to_uint_cmp val_limbs_ge0).
qed.
...
proof.
elim: x y => //=.
 by move=> y; rewrite add_limbs_nill max_ler //; apply size_ge0.
move=> x xs IH; elim => //=.
 by rewrite max_lel; smt(size_ge0).
...
lemma nosmt val_limbs64_cons_ltb x xs y ys b:
 val_limbs64 (x::xs) < val_limbs64 (y::ys) + b2i b 
 = (val_limbs64 xs < val_limbs64 ys
    \/ val_limbs64 xs = val_limbs64 ys /\ to_uint x < to_uint y + b2i b).
proof.
have AUXlt: forall x1 x2 m y1 y2,
 0 < m => 0 <= x1 < m => 0 <= x2 < m => 0 <= y1 => 0 <= y2 =>
 (x1 + m * y1 < x2 + m * y2) = (y1 < y2 \/ y1=y2 /\ x1 < x2) by smt().
have AUXle: forall x1 x2 m y1 y2,
 0 < m => 0 <= x1 < m => 0 <= x2 < m => 0 <= y1 => 0 <= y2 =>
 (x1 + m * y1 <= x2 + m * y2) = (y1 < y2 \/ y1=y2 /\ x1 <= x2) by smt().
rewrite !val_limbs64_cons. 
move: (W64.to_uint_cmp x) (W64.to_uint_cmp y) => *.
case: b => E; rewrite ?b2i1 ?b2i0 => *.
 rewrite !ltzS AUXle //; first 2 smt(val_limbs_ge0).
by rewrite /= AUXlt //; first 2 smt(val_limbs_ge0).
qed.
...
proof.
elim: x c => //= x xs IH c H0 /=.
rewrite /nth_limbs64 /nth_digits /=; case: (size xs = 0) => /= E ?.
 rewrite size_eq0 in E; rewrite E carryprop_limbs64_nil /val_digits /=.
 move=> ?; split; first by rewrite to_uintD_small of_uintK; smt(modz_small).
...
(* Non redundant subtraction *)

op borrowpropl_limbs64 (x: W64.t list) (b: bool) : bool * W64.t list =
 with x = "[]" => (b, [])
 with x = (::) y ys =>
  let (b',ys') = borrowpropl_limbs64 ys (borrow_sub y W64.zero b)
  in (b', (y - W64.of_int (b2i b))::ys').

lemma borrowpropl_limbs64_nil b: borrowpropl_limbs64 [] b = (b, []) by done.

lemma borrowpropl_limbs64_cons x xs b:
 borrowpropl_limbs64 (x::xs) b
 = let (b',xs') = borrowpropl_limbs64 xs (borrow_sub x W64.zero b)
   in (b', (x - W64.of_int (b2i b))::xs') by done.

lemma size_borrowpropl_limbs64 x b:
 size (borrowpropl_limbs64 x b).`2 = size x.
proof.
elim: x b => //= x xs IH b.
by rewrite -(IH (borrow_sub x W64.zero b)); case: (borrowpropl_limbs64 xs (borrow_sub x W64.zero b)).
qed.

lemma borrowpropl_limbs64P x b:
 (let (b',z) = borrowpropl_limbs64 x b
 in val_limbs64 z - 2^(64*size x) * b2i b') = val_limbs64 x - b2i b.
proof.
elim: x b => //= x xs IH b.
have := (IH (borrow_sub x W64.zero b)).
case: (borrowpropl_limbs64 xs (borrow_sub x W64.zero b)) => ?? /=.
rewrite !val_digits_cons.
have := W64.subcP x W64.zero b.
rewrite subcE /= => E1 E2.
have ->: 2 ^ (64 * (1 + size xs)) * b2i x1 = 2^64*2^(64*size xs)*b2i x1.
 by rewrite mulzDr pow_add //; smt(size_ge0).
smt().
...
(*
lemma nosmt borrowpropl_limbs64_ncP x b:
 0 < val_limbs64 x =>
 val_limbs64 (borrowpropl_limbs64 x b).`2 = val_limbs64 x - b2i b.
proof.
elim: x b => //= x xs; first by smt().
move=> IH b; rewrite val_digits_cons => H0.
case: (to_uint x = 0) => H1; last first.
 admit.
have ?: 0 < val_limbs64 xs by smt().

have := IH (borrow_sub x W64.zero b) H.
case: (borrowpropl_limbs64 _ _) => /= ??. 
rewrite !val_digits_cons H2.
have ->: b2i (borrow_sub x W64.zero b) = 0. admit.
rewrite /= to_uintB.
 admit.
...
op borrowpropr_limbs64 (x: W64.t list) (b: bool) : bool * W64.t list =
 with x = "[]" => (b, [])
 with x = (::) y ys =>
  let (b',ys') = borrowpropr_limbs64 ys (borrow_sub W64.zero y b)
  in (b', (-(y + W64.of_int (b2i b)))::ys').

lemma borrowpropr_limbs64_nil b: borrowpropr_limbs64 [] b = (b, []) by done.

lemma borrowpropr_limbs64_cons x xs b:
 borrowpropr_limbs64 (x::xs) b
 = let (b',xs') = borrowpropr_limbs64 xs (borrow_sub W64.zero x b)
   in (b', (-(x + W64.of_int (b2i b)))::xs') by done.

lemma size_borrowpropr_limbs64 x b:
 size (borrowpropr_limbs64 x b).`2 = size x.
proof.
elim: x b => //= x xs IH b.
by rewrite -(IH (borrow_sub W64.zero x b));
   case: (borrowpropr_limbs64 xs (borrow_sub W64.zero x b)).
qed.

lemma borrowpropr_limbs64P x b:
 (let (b',z) = borrowpropr_limbs64 x b
 in val_limbs64 z - 2^(64*size x) * b2i b') = -(val_limbs64 x + b2i b).
proof.
elim: x b => //= x xs IH b.
have := (IH (borrow_sub W64.zero x b)).
case: (borrowpropr_limbs64 xs (borrow_sub W64.zero x b)) => ?? /=.
rewrite !val_digits_cons.
have := W64.subcP W64.zero x b.
rewrite subcE /= => E1 E2.
have ->: 2 ^ (64 * (1 + size xs)) * b2i x1 = 2^64*2^(64*size xs)*b2i x1.
 by rewrite mulzDr pow_add //; smt(size_ge0).
smt().
...
op sub_limbs64 (x y: W64.t list) (b: bool) : bool * W64.t list =
with x = "[]", y = "[]" => (b, [])
with x = "[]", y = (::) y' ys => borrowpropr_limbs64 y b
with x = (::) x' xs, y = "[]" => borrowpropl_limbs64 x b
with x = (::) x' xs, y = (::) y' ys =>
 let z = W64.subc x' y' b in 
 let r = sub_limbs64 xs ys z.`1 in
 (r.`1, z.`2::r.`2).

lemma sub_limbs64_nill y b:
 sub_limbs64 [] y b = borrowpropr_limbs64 y b
by case: y.

lemma sub_limbs64_nilr x b:
 sub_limbs64 x [] b = borrowpropl_limbs64 x b
by case: x.

lemma sub_limbs64_cons x xs y ys b:
 sub_limbs64 (x::xs) (y::ys) b =
 let (b',z') = sub_limbs64 xs ys (borrow_sub x y b)
 in (b', (x-(y+W64.of_int (b2i b))) :: z').
proof. by rewrite /= !subcE /=; case: (sub_limbs64 xs ys (borrow_sub x y b)). qed.


lemma size_sub_limbs64 x y c:
size (sub_limbs64 x y c).`2 = MAX (size x) (size y).
proof.
elim: x y c => //=.
elim => //= y ys IH c.
have:= size_borrowpropr_limbs64 ys (borrow_sub W64.zero y c).
case: (borrowpropr_limbs64 ys (borrow_sub W64.zero y c)) => /= ??. 
 smt(size_ge0). 
move=> x xs IH; elim => //= [|y ys IH2] c.
have:= size_borrowpropl_limbs64 xs (borrow_sub x W64.zero c).
by case: (borrowpropl_limbs64 xs (borrow_sub x W64.zero c)) => /= ??; 
 smt(size_ge0).
smt().
qed.

lemma nosmt sub_limbs64P x y b:
 let (b',z) = sub_limbs64 x y b in
 val_limbs64 z = val_limbs64 x - (val_limbs64 y + b2i b)
                 + 2^(64*MAX (size x) (size y)) * b2i b'.
proof.
elim: x y b => /= [|x xs IH].
 move=> y c; rewrite !sub_limbs64_nill val_digits_nil /=.
 have ->: MAX 0 (size y) = size y by smt(size_ge0).
 move: (borrowpropr_limbs64P y c).
 by rewrite (Core.pairS (borrowpropr_limbs64 _ _)) //= /#. 
elim => /=.
 move=> {IH} c; rewrite !val_digits_cons /=.
 have ->: MAX (1 + size xs) 0 = 1 + size xs by smt(size_ge0).
 move: (borrowpropl_limbs64P xs (borrow_sub x W64.zero c)).
 rewrite (Core.pairS (borrowpropl_limbs64 _ _)) //=.
 move: (W64.subcP x W64.zero c); rewrite subcE /= => E1 E2.
 by rewrite !val_digits_cons mulzDr -pow_add; smt(size_ge0).
move=> y ys IH2 c; rewrite !val_digits_cons !subcE /=.
have ->: MAX (1 + size xs) (1 + size ys) = 1 + MAX (size xs) (size ys) by smt(size_ge0).
rewrite mulzDr -pow_add /=; first 2 smt(size_ge0).
move: (IH ys (borrow_sub x y c)); rewrite (Core.pairS (sub_limbs64 _ _ _ )) //=. 
case: (sub_limbs64 xs ys (borrow_sub x y c)) => /= ?? E.
by move: (W64.subcP x y c); rewrite subcE /= => ?; smt().
qed.


op borrow_sub_limbs64 xs ys b = val_limbs64 xs < val_limbs64 ys + b2i b.

op val_sub_limbs64 xs ys b =
 val_limbs64 xs - (val_limbs64 ys + b2i b)
 + 2^(64*MAX (size xs) (size ys)) * b2i (borrow_sub_limbs64 xs ys b).

lemma nosmt borrowpropr_limbs64E1 ys b:
 (borrowpropr_limbs64 ys b).`1 = 0 < val_limbs64 ys + b2i b.
proof.
elim: ys b => //=; first smt().
move=> y ys IH b /=.
rewrite (Core.pairS (borrowpropr_limbs64 ys (borrow_sub W64.zero y b))).
rewrite IH /borrow_sub val_digits_cons.
rewrite {1}/b2i to_uint0.
case: (0 < to_uint y + b2i b); case: b; rewrite ?b2i1 ?b2i0 /=.
   rewrite !ltzS => *.
   case: (0 <= val_limbs 18446744073709551616 ys) => *; first smt().
   smt(val_limbs_ge0).
  rewrite !ltzS => *.
  case: (0 <= val_limbs 18446744073709551616 ys) => *; first smt().
  smt(val_limbs_ge0).
...
lemma nosmt borrowpropl_limbs64E1 xs b:
 (borrowpropl_limbs64 xs b).`1 = val_limbs64 xs < b2i b.
elim: xs b => //=; first smt().
move=> x xs IH b /=.
rewrite (Core.pairS (borrowpropl_limbs64 xs (borrow_sub x W64.zero b))).
rewrite IH /borrow_sub val_digits_cons.
rewrite {1}/b2i add0z.
case: (to_uint x < b2i b); case: b; rewrite ?b2i1 ?b2i0 /=;
smt(W64.to_uint_cmp val_limbs_ge0). 
qed.

lemma sub_limbs64E1 x y b:
 (sub_limbs64 x y b).`1 = borrow_sub_limbs64 x y b.
proof.
elim: x y b => //= [ys b|x xs IH].
 rewrite sub_limbs64_nill /borrow_sub_limbs64 val_digits_nil.
 by apply borrowpropr_limbs64E1.
elim => /= [|y ys IH2] b.
 rewrite /borrow_sub_limbs64 val_digits_nil /=.
 rewrite (Core.pairS (borrowpropl_limbs64 xs (borrow_sub x W64.zero b))) /=.
 rewrite /borrow_sub val_digits_cons borrowpropl_limbs64E1 to_uint0 /=.
 case: b => E; rewrite ?b2i1 ?b2i0 /=; smt(W64.to_uint_cmp val_limbs_ge0). 
rewrite /borrow_sub_limbs64 IH; clear IH IH2.
rewrite val_limbs64_cons_ltb. 
case: b => E; rewrite /borrow_sub_limbs64 subcE /= /borrow_sub ?b2i0 ?b2i1 /=.
 by case: (to_uint x < to_uint y + 1) => E1 /= /#.
by case: (to_uint x < to_uint y) => E1 /= /#.
qed.

lemma sub_limbs64val x y b:
 val_limbs64 (sub_limbs64 x y b).`2 = val_sub_limbs64 x y b.
proof.
move: (sub_limbs64P x y b).
rewrite (Core.pairS (sub_limbs64 x y b)) sub_limbs64E1 /= => ->.
by rewrite /val_sub_limbs64 /#. 
qed.


op sub_limbs64nb l1 l2 = (sub_limbs64 l1 l2 false).`2.
op sub_limbs64nbc l1 l2 = (sub_limbs64 l1 l2 false).`1.

lemma size_sub_limbs64nb x y:
 size (sub_limbs64nb x y) = MAX (size x) (size y).
proof. smt(size_sub_limbs64). qed.

lemma nosmt sub_limbs64nbP x y:
 val_limbs64 y <= val_limbs64 x =>
 val_limbs64 (sub_limbs64nb x y) = val_limbs64 x - val_limbs64 y
 /\ sub_limbs64nbc x y = false.
proof.
rewrite /sub_limbs64nb sub_limbs64val /val_sub_limbs64 /sub_limbs64nbc => ?.
rewrite sub_limbs64E1 /borrow_sub_limbs64 ?b2i0 addz0.
have ->: (val_limbs64 x < val_limbs64 y) = false.
 by rewrite ltzNge H.
smt().
qed.
