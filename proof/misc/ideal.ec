...
lemma sumIsId : forall i j , ideal i => ideal j => ideal (idealD i j).
move => i j idi idj.
apply : idealP.
...
lemma quoIdIsId : forall i j , ideal i => ideal j => ideal (idealQ i j).
move => i j idi idj.
apply : idealP.
+ exists zeror.
  move => x jx.
...
lemma genIsId : forall i , ideal (ideal_gen i).
move => i.
apply : idealP.
+ by exists zeror [].
...
lemma genIdContainsSet : forall i , forall x , i x => ideal_gen i x.
move => i x ix.
exists [(x,oner)].
split => //.
...
lemma genIdSmallestIdContainingSet : forall i j , ideal j => (forall x , i x => j x) => (forall x , ideal_gen i x => j x).
...
lemma prodIdIsId : forall i j , ideal i => ideal j => ideal (idealM i j).
move => i j idi idj.
...
lemma radIdIsId : forall i , ideal i => ideal (ideal_rad i).
move => i idi.
apply : idealP.
...
+ move => x y [nx [le0nx iexnx]] [ny [le0ny ieyny]].
  ...
  - have eqf: (\o) idfun (fun (i0 : int) => intmul (exp x i0 * exp y (nx + ny - i0)) (bin (nx + ny) i0)) = (fun (i0 : int) => intmul (exp x i0 * exp y (nx + ny - i0)) (bin (nx + ny) i0)).
    ...
    * apply mem_idealMr => //.
      rewrite - addzAC.
      by rewrite (exprD y (nx-r) ny)  // ; [rewrite subz_ge0 | apply mem_idealMr].
    * apply mulir => //.
