...
lemma size_decode k n l :
 1 <= k <= 4  =>
 1 <= n <= 12 => size l = k*32*n =>  size (ByteDecode n l) = k*256.
move => Hn Hsize.
rewrite /ByteDecode size_map size_chunk 1:/# /BytesToBits (EclibExtra.size_flatten' 8);1:smt(mapP W8.size_w2bits).
by rewrite size_map /#.
qed.
...
rewrite of_listK;1: by rewrite (size_decode 1 1); [by smt() | by smt()| by rewrite Array32.size_to_list /= | ].
rewrite -decodeK 1:/#;1:smt().
by apply Array32.to_listK.
qed.

lemma sem_decode_polyK  : cancel decode_poly  encode_poly.
have dvb : 1 <= dv <= 12 by smt(gt0_dv dv_le12).
rewrite /cancel /encode_poly /decode_poly => x.
rewrite of_listK;1: by rewrite (size_decode 1 dv); [by smt() | by smt()|  by rewrite BytesPoly.size_to_list /= | ].
rewrite -decodeK 1:/#;1: smt(BytesPoly.size_to_list).
by apply BytesPoly.to_listK.
qed.

lemma sem_decode12K  : cancel decode12  encode12.
rewrite /cancel /encode12 /decode12 => x.
rewrite of_listK;1: by rewrite (size_decode 1 12); [by smt() | by smt()|  by rewrite Array384.size_to_list /= | ].
...
apply tP => k kb.
rewrite get_of_list //.
rewrite of_listK /=;1: by rewrite (size_decode kvec 12); smt(gt0_k param_sets BytesPKVec.size_to_list).
rewrite -decodeK 1:/#;1: smt(BytesPKVec.size_to_list).
rewrite (nth_change_dfl witness W8.zero);1: smt(BytesPKVec.size_to_list).
by smt().
qed.
...
lemma sem_decode_vecK  : cancel decode_vec  encode_vec.
have dub : 1 <= du <= 12 by smt(gt0_du du_le12).
have kb4 : 1 <= kvec <= 4 by smt(gt0_k param_sets).
rewrite /cancel /encode_vec /decode_vec => x.
apply BytesCtVec.tP => k kb.
rewrite get_of_list //.
rewrite of_listK /=;1: by rewrite (size_decode kvec du); smt(gt0_k gt0_du du_le12 param_sets BytesCtVec.size_to_list).
rewrite -decodeK 1:/#;1: by smt(BytesCtVec.size_to_list param_sets).
rewrite (nth_change_dfl witness W8.zero);1: smt(BytesCtVec.size_to_list).
by smt().
qed.
...
rewrite get_of_list // of_listK; 1: by rewrite (size_encode 1 1) /=;  [by smt() | by smt()|  by rewrite Array256.size_to_list /= | ].
...
lemma sem_encode_polyK (x : ipoly) :
   (forall i, 0 <= i < 256 => 0 <= x.[i] < 2^dv) =>
     x =  decode_poly (encode_poly x).
have dvb : 1 <= dv <= 12 by smt(gt0_dv dv_le12).
move => Hi;rewrite /encode_poly /decode_poly.
apply Array256.tP => k kb.
rewrite get_of_list // of_listK; 1: by rewrite (size_encode 1 dv) /=;  [by smt() | by smt()|  by rewrite Array256.size_to_list /= | ].
rewrite -encodeK 1:/#;[smt(Array256.size_to_list) | rewrite /= allP /= /to_list;smt(mkseqP) | ].
rewrite -(get_to_list ).
by rewrite (nth_change_dfl witness 0);1: smt(Array256.size_to_list).
qed.

lemma sem_encode_vecK (x : ipolyvec) :
   (forall i, 0 <= i < 256*kvec => 0 <= x.[i] < 2^du) =>
     x =  decode_vec (encode_vec x).
have dub : 1 <= du <= 12 by smt(gt0_du du_le12).
have kb4 : 1 <= kvec <= 4 by smt(gt0_k param_sets).
move => Hi;rewrite /encode_vec /decode_vec.
apply IPVec.tP => k kb.
rewrite get_of_list // of_listK; 1: by rewrite (size_encode kvec du); smt(gt0_k gt0_du du_le12 param_sets IPVec.size_to_list).
rewrite -encodeK 1:/#;[smt(IPVec.size_to_list) | rewrite /= allP /= /to_list;smt(mkseqP) | ].
rewrite -(get_to_list ).
by rewrite (nth_change_dfl witness 0);1: smt(IPVec.size_to_list).
qed.

lemma sem_encode12_vecK (x : ipolyvec) :
   (forall i, 0 <= i < 256*kvec => 0 <= x.[i] < 2^12) =>
     x =  decode12_vec(encode12_vec x).
move => /= Hi;rewrite /encode12_vec /decode12_vec.
apply IPVec.tP => k kb.
rewrite get_of_list // of_listK; 1: by rewrite (size_encode kvec 12); smt(gt0_k param_sets IPVec.size_to_list).
rewrite -encodeK 1:/#.
+ smt(IPVec.size_to_list param_sets).
rewrite /= allP /= /to_list => kk /=.
rewrite mkseqP => H;elim H;1: smt(param_sets).
by rewrite -(get_to_list ) (nth_change_dfl witness 0);1: smt(IPVec.size_to_list).
qed.
...
proof.
move => ?; rewrite /decode1 get_of_list // /ByteDecode (nth_map witness).
