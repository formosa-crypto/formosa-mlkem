...
equiv genmatrixequiv b :
 Jkem1024_avx2.M(Jkem1024_avx2.Syscall)._gen_matrix_avx2 ~ Jkem1024.M(Jkem1024.Syscall).__gen_matrix :
...
module GetNoiseAVX2 = {
  proc _poly_getnoise_eta1_4x(aux3 aux2 aux1 aux0 : W16.t Array256.t,
      ...
      n3 <- nonce;
      aux_3 <@Jkem1024.M(Jkem1024.Syscall)._poly_getnoise(aux3,noiseseed,n3);
      aux_2 <@Jkem1024.M(Jkem1024.Syscall)._poly_getnoise(aux2,noiseseed,n2);
      aux_1 <@Jkem1024.M(Jkem1024.Syscall)._poly_getnoise(aux1,noiseseed,n1);
      aux_0 <@Jkem1024.M(Jkem1024.Syscall)._poly_getnoise(aux0,noiseseed,n0);
...
nonce <- W8.of_int 8;                                          
epp <@                                                    
  Jkem1024.M(Jkem1024.Syscall)._poly_getnoise(epp,     
    noiseseed, nonce);      
return (sp_0,ep,epp);
  }
}.
...
hoare cbd2_avx2_h _bytes:
 Jkem1024_avx2.M(Jkem1024_avx2.Syscall).__cbd2: buf=_bytes ==> res = Array256.init (fun k => W16.of_int (noise_coef _bytes k)).
proof.
proc.
sp; simplify.
...
lemma cbd2_ll : islossless Jkem1024_avx2.M(Jkem1024_avx2.Syscall).__cbd2.
proc. inline *. sp; wp. while (true) (4-i). move => z.
auto => /> &hr H. smt().
auto => />i. smt(). qed. 

phoare cbd2_avx2_ph _bytes:
 [Jkem1024_avx2.M(Jkem1024_avx2.Syscall).__cbd2: buf=_bytes ==> res = Array256.init (fun k => W16.of_int (noise_coef _bytes k))] = 1%r.
...
equiv getnoise_1x_equiv_avx :
  Jkem1024_avx2.M(Jkem1024_avx2.Syscall).__poly_cbd_eta1 ~ AuxMLKEMAvx2.cbd2_ref : ={arg} ==> ={res}.
proc*. inline Jkem1024_avx2.M(Jkem1024_avx2.Syscall).__poly_cbd_eta1.  sp;wp.
...
equiv getnoiseequiv_avx : 
   Jkem1024_avx2.M(Jkem1024_avx2.Syscall)._poly_getnoise_eta1_4x ~ GetNoiseAVX2._poly_getnoise_eta1_4x : ={arg} ==> ={res}.
proc*. 
...
(*main proof*)
inline Jkem1024_avx2.M(Jkem1024_avx2.Syscall)._poly_getnoise_eta1_4x AuxMLKEMAvx2.__poly_getnoise_eta1_4x AuxMLKEMAvx2._poly_getnoise. swap{2} [30..31] 5. swap{2} [23..24] 10. swap{2} [16..17] 15.
seq 27 30 : (
...
lemma polygetnoise_ll : islossless Jkem1024.M(Jkem1024.Syscall)._poly_getnoise.
proc. 
...
equiv getnoiseequiv : 
  Jkem1024.M(Jkem1024.Syscall)._poly_getnoise ~Jkem1024.M(Jkem1024.Syscall)._poly_getnoise :
   ={arg} ==> ={res} /\
   signed_bound_cxq res{1} 0 256 1.
have H : forall &m a,
   Pr[Jkem1024.M(Jkem1024.Syscall)._poly_getnoise(a) @ &m : forall k, 0<=k<256 => -5 < to_sint res.[k] < 5] = 1%r.
+ move => &m a.
...
have HH0 : hoare [Jkem1024.M(Jkem1024.Syscall)._poly_getnoise : true ==> forall k, 0<=k<256 => -5 < to_sint res.[k] < 5].
+ hoare; bypr =>  &m; rewrite Pr[mu_not].
  have -> : Pr[Jkem1024.M(Jkem1024.Syscall)._poly_getnoise(rp{m}, s_seed{m}, nonce{m}) @ &m : true] = 1%r.
  + by byphoare;2..:smt(); apply polygetnoise_ll.
  smt().
have HHH : equiv [ Jkem1024.M(Jkem1024.Syscall)._poly_getnoise ~Jkem1024.M(Jkem1024.Syscall)._poly_getnoise : ={arg} ==> ={res} ] by sim.
conseq HHH HH0.
move => *; rewrite /signed_bound_cxq /b16 qE /#.
qed.

equiv getnoiseequiv2 : 
   Jkem1024_avx2.M(Jkem1024_avx2.Syscall)._poly_getnoise_eta2 ~M(Syscall)._poly_getnoise :
   ={arg} ==> ={res}  /\
   signed_bound_cxq res{1} 0 256 1.
symmetry.
proc*. 
...
+ inline {1} 1. inline {2} 1. 
  ...
  sp;conseq/>.
  admit. 
transitivity{1} { r <@ M(Syscall)._poly_getnoise(rp,seed,nonce); } 
     ( ={rp,seed,nonce}  ==> ={r})
    ( ={rp,nonce} /\ seed{1} = s_seed{2} ==> ={r} /\  signed_bound_cxq r{1} 0 256 1);1,2:smt().
...
proc*.
transitivity {1} {Jkem1024.M(Jkem1024.Syscall).__indcpa_keypair(pkp, skp, randomnessp);} 
...
+ conseq />.
  transitivity {1} { (skpv,e) <@ GetNoiseAVX2.sample_noise_kg(skpv,e,noiseseed);} (={noiseseed,skpv,pkpv,e} ==> ={skpv,e}) 
   ...
   (spkp{2} = pkp{2} /\
    ...
    Glob.mem{1} = mem /\
    to_uint pkp{1} = _pkp /\
    to_uint skp{1} = _skp /\ ={randomnessp} /\ valid_disj_reg _pkp (384 * 4 + 32) _skp (384 * 4)) /\
   ={publicseed, noiseseed, e, skpv, pkpv} /\ sskp{2} = skp{1} /\ spkp{2} = pkp{1}) /\
  aa{1} = nttunpackm a{2} /\ pos_bound4096_cxq aa{1} 0 4096 2 /\ pos_bound4096_cxq a{2} 0 4096 2
  ...
  wp;do 8!(wp; call  getnoiseequiv); auto => />.
  move => &1 &2 ??????R?; split.
...
+ rewrite /unpackm /unpackv /=.
  ...
  + move => kbb;rewrite /subarray256. 
    move : (nttunpack_pred (Array256.init (fun (k0 : int) => (subarray1024 a{2} 0).[256 * 0 + k0])) (fun x => -2*q <= W16.to_sint x < 2*q)).
    rewrite !allP; move => /= [h0 h1]; rewrite  h1. move => *. rewrite initiE //=. smt(Array1024.initiE). smt().
  case (256<=k<512).
  + move => kbb;rewrite /subarray256. 
    move : (nttunpack_pred (Array256.init (fun (k0 : int) => (subarray1024 a{2} 0).[256 * 1 + k0])) (fun x => -2*q <= W16.to_sint x < 2*q)).
    rewrite !allP; move => /= [h0 h1]; rewrite  h1. move => *. rewrite initiE //=. smt(Array1024.initiE). smt(). auto.
  case (512<=k<768).
  + move => kbbb;rewrite /subarray256. 
    move : (nttunpack_pred (Array256.init (fun (k0 : int) => (subarray1024 a{2} 0).[256 * 2 + k0])) (fun x => -2*q <= W16.to_sint x < 2*q)).
    rewrite !allP; move => /= [h0 h1]; rewrite  h1. move => *. rewrite initiE //=. smt(Array1024.initiE). smt(). auto.
  case (768<=k<1024).
  + move => kbbbb;rewrite /subarray256. 
    move : (nttunpack_pred (Array256.init (fun (k0 : int) => (subarray1024 a{2} 0).[256 * 3 + k0])) (fun x => -2*q <= W16.to_sint x < 2*q)).
    rewrite !allP; move => /= [h0 h1]; rewrite  h1. move => *. rewrite initiE //=. smt(Array1024.initiE). smt(). auto.
  by smt().
+ move : H10; rewrite /pos_bound1024_cxq /signed_bound_1024_cxq /#.
+ move : H8; rewrite /pos_bound1024_cxq /signed_bound_1024_cxq; smt(Array1024.initiE).
+ move : H10; rewrite /pos_bound1024_cxq /signed_bound_1024_cxq; smt(Array1024.initiE).
...
wp; call frommontequiv; wp; call pointwiseequiv; auto => />.
move => &1 &2 H2 H3 H4 H5 H6 H7 H8 H9 H10 H11 H12 H13 H14 H15 H16 H17 H18 H19 H20 H221 H222 H223; do split.
+ rewrite -lift1024_nttunpack. congr.
  rewrite /nttunpackm /nttunpackv tP /= => k kb.
...
+ rewrite tP /= => k kb.
  ...
  rewrite ifF;1:smt(nttunpack_bnd Array256.allP).
  move : H221; rewrite /lift_array256 /subarray256 tP => H221.
  move : (H221 k kb).
...
lemma mlkem_correct_enc_0_avx2 mem _ctp _pkp : 
   equiv [Jkem1024_avx2.M(Jkem1024_avx2.Syscall).__indcpa_enc_0 ~ InnerPKE1024.enc_derand: 
...
proc*.
transitivity {1} {Jkem1024.M(Jkem1024.Syscall).__indcpa_enc(sctp,msgp,pkp,noiseseed);} 
...
seq 54 67 : (={ctp,Glob.mem} /\
     pos_bound256_cxq v{1} 0 256 2 /\
     pos_bound256_cxq v{2} 0 256 2 /\
    lift_array256 v{1} = lift_array256 v{2} /\ 
    valid_ptr (to_uint ctp{1}) 160); last by
  exists *Glob.mem{1}, (to_uint ctp{1}); elim* => memm _p; call (compressequiv memm _p); auto.

seq 52 65 : (={ctp,Glob.mem} /\
     pos_bound256_cxq v{1} 0 256 2 /\
     pos_bound256_cxq v{2} 0 256 2 /\
     pos_bound1024_cxq bp{1} 0 1024 2 /\
     pos_bound1024_cxq bp{2} 0 1024 2 /\
    lift_array256 v{1} = lift_array256 v{2} /\ 
    lift_array1024 bp{1} = lift_array1024 bp{2} /\ 
    valid_ptr (to_uint ctp{1}) (160+4*352)); last by
  exists *Glob.mem{1}, (to_uint ctp{1}); elim* => memm _p; wp;call (compressequivvec memm _p); auto => />; move => *; rewrite !to_uintD_small /= /#.
...
+ call (genmatrixequiv true).
  ...
  conseq (_: _ ==> lift_array1024 pkpv{1} = nttunpackv (lift_array1024 pkpv{2}) /\
       pos_bound1024_cxq pkpv{1} 0 1024 2 /\ pos_bound1024_cxq pkpv{2} 0 1024 2 /\ ={publicseed,pkp0,bp,ep,epp,v,sp_0,Glob.mem} /\ pkp0{2} = pkp{1} /\ s_noiseseed{2} = noiseseed0{2}).
...
seq 16 26 : (#{/~bp{1}=bp{2}}{~lnoiseseed{1}}pre  /\
...
+ conseq />.
  ...
  (s_noiseseed{1} = noiseseed{2} /\ noiseseed0{1} = noiseseed{2} /\ lnoiseseed{1} = noiseseed{2} /\  ={sp_0,ep,bp,epp} ==> ={sp_0,ep,epp}) 
   (
  ...
  (sctp0{2} = sctp{2} /\
   ...
   (={Glob.mem, msgp, noiseseed, pkp, sctp} /\
    valid_ptr _pkp (384 * 4 + 32) /\
    valid_ptr _ctp (4 * 352 + 160) /\ Glob.mem{1} = mem /\ to_uint sctp{1} = _ctp /\ to_uint pkp{1} = _pkp) /\
   ={pkp0}) /\
  ...
  do 9!(wp; call  getnoiseequiv); auto => />.
  move => &1 &2 /> ??????????R?; split.
  + by rewrite tP => k kb; rewrite !initiE //= initiE /#.
  move => ?R0?; split.
  + rewrite tP => k kb; rewrite !initiE //= initiE 1:/# /= initiE 1:/# /= /#.
  move => ?R1?; split.
  + rewrite tP => k kb; rewrite !initiE //= initiE 1:/# /= initiE 1:/# /= ifF 1:/# ifF 1:/# initiE /#.
  move => ?R2???; split.
  + rewrite tP => k kb; rewrite !initiE //= initiE 1:/# /= /= /#.
  move => ?R3?; split.
  + rewrite tP => k kb; rewrite !initiE //= initiE 1:/# /=  initiE 1:/# /= /= /#.
  move => ?R4?; split.
  + rewrite tP => k kb; rewrite !initiE //= initiE 1:/# /= ifF 1:/# /= initiE 1:/# /= ifF 1:/# initiE 1:/# /= /= /#.
  move => ?R5???; do split.
  + rewrite /signed_bound1024_cxq => x xb /=.
    rewrite !initiE //= fun_if. 
    case (768 <= x && x < 1024); 1: by smt().
...
wp; call pointwiseequiv; auto => />.
move => &1 &2 H0 H1 H2 H3 H4 H5 H6 H7 H8 H9 H10 H11 H12 H13 H14 H15 H16 H17 H181 H191 H201; do split.
+ rewrite -lift1024_nttunpack. congr.
  rewrite /nttunpackm /nttunpackv tP /= => k kb.
  rewrite !initiE //= initiE //= 1:/# ifF 1:/# ifF 1:/# ifT 1:/# initiE //=. 
+ by rewrite /signed_bound1024_cxq => k kb; rewrite initiE //= /#. 
...
+ rewrite tP /= => k kb.
  ...
  have ? /= :  0 <= a && a < 256  by smt(nttunpack_bnd Array256.allP).
  rewrite ifF 1: /# ifF 1: /# /=. 
  move : H181; rewrite /lift_array256 /subarray256 tP => H181.
  move : (H181 k kb). 
...
wp; call pointwiseequiv; auto => />.
move => &1 &2 H0 H1 H2 H3 H4 H5 H6 H7 H8 H9 H10 H11 H12 H13 H14 H15 H16 H17 H18 H19 H20 H211 H221 H222; do split.
+ rewrite -lift1024_nttunpack. congr.
  rewrite /nttunpackm /nttunpackv tP /= => k kb.
  rewrite !initiE //= initiE //= 1:/# ifF 1:/# ifF 1:/# ifF 1:/# initiE //=. 
+ by rewrite /signed_bound1024_cxq => k kb; rewrite initiE //= /#. 
...
+ rewrite tP /= => k kb.
  ...
  have ? /= :  0 <= a && a < 256  by smt(nttunpack_bnd Array256.allP).
  rewrite ifF 1: /# ifF 1: /# /=. 
  move : H211; rewrite /lift_array256 /subarray256 tP => H211.
  move : (H211 k kb). 
  rewrite /nttunpack initiE //= -/a !mapiE //=.
  by smt(nttunpack_bnd Array256.allP Array256.initiE).  
...
wp; call pointwiseequiv; auto => />.
move => &1 &2 H0 H1 H2 H3 H4 H5 H6 H7 H8 H9 H10 H11 H12 H13 H14 H15 H16 H17 H18 H19 H20 H21 H22 H23 H241 H252 H261; do split.
...
lemma mlkem_correct_enc_1_avx2 mem _pkp : 
   equiv [Jkem1024_avx2.M(Jkem1024_avx2.Syscall).__indcpa_enc_1 ~ InnerPKE1024.enc_derand: 
     valid_ptr _pkp (384*4 + 32) /\
     Glob.mem{1} = mem /\ 
     msgp{1} = m{2} /\ 
     to_uint pkp{1} = _pkp /\
...
proc*.
transitivity {1} { r <@Jkem1024.M(Jkem1024.Syscall).__iindcpa_enc(ctp,msgp,pkp,noiseseed);} 
...
+ call (genmatrixequiv true).
  wp;call frommsgequiv_noperm. conseq />. smt().
  conseq (_: _ ==> lift_array1024 pkpv{1} = nttunpackv (lift_array1024 pkpv{2}) /\ ctp{2} = sctp{2} /\
       pos_bound1024_cxq pkpv{1} 0 1024 2 /\ pos_bound1024_cxq pkpv{2} 0 1024 2 /\ ={publicseed,pkp0,bp,ep,epp,v,sp_0,Glob.mem} /\ pkp0{2} = pkp{1} /\ s_noiseseed{2} = noiseseed0{2}).
  auto => /> &2 ??????? rl rr H H0 H1 ????. 
  + rewrite tP => k kb.
    move : H; rewrite /lift_array256 tP => H.
...
+ conseq />.
  transitivity {1} { (sp_0,ep,epp) <@ GetNoiseAVX2.samplenoise_enc(sp_0,ep, epp,noiseseed);} 
    (noiseseed0{1} = noiseseed{2} /\ s_noiseseed{1} = noiseseed{2} /\ lnoiseseed{1} = noiseseed{2} /\ ={sp_0,ep,bp,epp} ==> ={sp_0,ep,epp}) 
   (
   s_noiseseed{1} = noiseseed0{1} /\
  lnoiseseed{1} = s_noiseseed{1} /\
  (ctp0{2} = ctp{2} /\
...
seq 3 2: (#{/~w{1}}pre /\ lift_array256 (subarray256 bp{1} 2) = nttunpack (lift_array256 (subarray256 bp{2} 2)) /\
              ...
              signed_bound1024_cxq bp{2} 512 768 2 /\ w{1} = 3).
wp; call pointwiseequiv; auto => />.
move => &1 &2 H2 H3 H4 H5 H6 H7 H8 H9 H10 H11 H12 H13 H14 H15 H16 H17 H181 H191 H201; do split.
+ rewrite -lift1024_nttunpack. congr.
  rewrite /nttunpackm /nttunpackv tP /= => k kb.
  rewrite !initiE //= initiE //= 1:/# ifF 1:/# ifF 1:/# ifT 1:/# initiE //=. 
+ by rewrite /signed_bound1024_cxq => k kb; rewrite initiE //= /#. 
...
seq 3 2: (#{/~w{1}}pre /\ lift_array256 (subarray256 bp{1} 3) = nttunpack (lift_array256 (subarray256 bp{2} 3)) /\
              ...
              signed_bound1024_cxq bp{2} 768 1024 2 ).
wp; call pointwiseequiv; auto => />.
move => &1 &2 H2 H3 H4 H5 H6 H7 H8 H9 H10 H11 H12 H13 H14 H15 H16 H17 H18 H19 H20 H211 H221 H222; do split.
+ rewrite -lift1024_nttunpack. congr.
  rewrite /nttunpackm /nttunpackv tP /= => k kb.
  rewrite !initiE //= initiE //= 1:/# ifF 1:/# ifF 1:/# ifF 1:/# initiE //=. 
+ by rewrite /signed_bound1024_cxq => k kb; rewrite initiE //= /#. 
...
seq 1 1: (#{/~v{1}}pre /\ lift_array256 v{1} = nttunpack (lift_array256 v{2}) /\
              ...
              signed_bound_cxq v{2} 0 256 2).
wp; call pointwiseequiv; auto => />.
move => &1 &2 H2 H3 H4 H5 H6 H7 H8 H9 H10 H11 H12 H13 H14 H15 H16 H17 H18 H19 H20 H21 H22 H23 H241 H252 H261; do split.
+ by rewrite /signed_bound1024_cxq => k kb; smt(). 
+ by rewrite /signed_bound1024_cxq => k kb; smt(). 
+ by rewrite /signed_bound1024_cxq => k kb; smt(). 
+ by rewrite /signed_bound1024_cxq => k kb; smt(). 
...
lemma mlkem_correct_dec mem _ctp _skp : 
   equiv [Jkem1024_avx2.M(Jkem1024_avx2.Syscall).__indcpa_dec_1 ~ InnerPKE1024.dec : 
     valid_ptr _ctp (4*352+160) /\
     valid_ptr _skp 1568 /\
     Glob.mem{1} = mem /\ 
     to_uint ctp{1} = _ctp /\
...
proc*.
transitivity {1} { r <@Jkem1024.M(Jkem1024.Syscall).__indcpa_dec(msgp,ctp,skp);} 
(={Glob.mem,ctp,skp} /\
  valid_ptr _ctp (4*352+160) /\
     valid_ptr _skp 1536 /\
     Glob.mem{1} = mem /\ 
...
call(nttequiv).
call (polyvec_frombytes_equiv).
ecall (poly_decompress_equiv mem (_ctp + 4*352)).
