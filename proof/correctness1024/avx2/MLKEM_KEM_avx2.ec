...
lemma mlkem_kem_correct_kg mem _pkp _skp  : 
   equiv [Jkem1024_avx2.M(Jkem1024_avx2.Syscall).__crypto_kem_keypair_jazz ~ MLKEM1024.kg_derand : 
...
+ while {1} (inc{1} = 4 /\
  ...
  + rewrite tP => k kb; rewrite !initiE 1..4:/# /=. 
  +  smt().
  move => memL iL skpL.  
  split; 1: smt().
  move => ???touch2????????store???. 
...
ecall {1} (sha3_256A_M1568_ph (Glob.mem{1}) (_pkp)).
...
lemma mlkem_kem_correct_enc mem _ctp _pkp _kp : 
   equiv [Jkem1024_avx2.M(Jkem1024_avx2.Syscall).__crypto_kem_enc_jazz ~ MLKEM1024.enc_derand: 
...
wp; ecall {1} (sha3_256A_M1568_ph mem (_pkp)).
...
lemma verify_correct_h mem (_ctp : int) ctp1 :
  hoare [Jkem1024_avx2.M(Jkem1024_avx2.Syscall).__verify : 
...
lemma verify_ll : islossless Jkem1024_avx2.M(Jkem1024_avx2.Syscall).__verify.
proc.
seq 8 : (#post) => //; last first.
...
lemma verify_correct mem (_ctp : int) ctp1 :
  phoare [Jkem1024_avx2.M(Jkem1024_avx2.Syscall).__verify : 
             Glob.mem = mem /\ valid_ptr _ctp 1568 /\
             to_uint ctp = _ctp /\ ctpc = ctp1 ==>
             Glob.mem = mem /\
             (Array1568.init (fun i => loadW8 mem (_ctp + i)) = ctp1 => 
...
lemma cmov_correct_h _dst _src _cnd mem:
   hoare [Jkem1024_avx2.M(Jkem1024_avx2.Syscall).__cmov : 
...
lemma cmov_ll : islossless Jkem1024_avx2.M(Jkem1024_avx2.Syscall).__cmov.
proc => /=.
unroll 6.
...
lemma cmov_correct _dst _src _cnd mem:
   phoare [Jkem1024_avx2.M(Jkem1024_avx2.Syscall).__cmov : 
             Glob.mem = mem /\ valid_ptr _dst 32 /\
             src = _src /\ cnd = _cnd /\ to_uint dst = _dst ==>
             touches mem Glob.mem _dst 32 /\
             (_cnd = W64.of_int 0 => 
...
lemma mlkem_kem_correct_dec mem _ctp _skp _shkp : 
   equiv [Jkem1024_avx2.M(Jkem1024_avx2.Syscall).__crypto_kem_dec_jazz ~ MLKEM1024.dec: 
