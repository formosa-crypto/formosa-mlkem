...
from CryptoSpecs require import MLKEM1024.
from JazzEC require import Jkem1024_avx2 Jkem1024_avx2_stack.
from Jasmin require import JModel_x86.
from JazzEC require import Array32 Array33 Array64 Array148 Array256 Array384 Array1024 Array1408 Array128 Array1600 Array1536 Array1568 Array4096 Array2400.
from JazzEC require import Array8 WArray32 WArray33 Array300 WArray64 WArray1536 WArray1568 WArray2400 WArray384.
require import MLKEM_InnerPKE_avx2.
import MLKEM1024 InnerPKE1024.

require import MLKEMFCLib.

op load_array1568 (m : global_mem_t) (p : address) : W8.t Array1568.t = 
      (Array1568.init (fun (i : int) => m.[p + i])).

lemma poly_to_bytes_stack_equiv _mem _pos _a :
   0 <= _pos <= 1568+3*384 =>
   equiv [ Jkem1024_avx2_stack.M._i_poly_tobytes
           ~ Jkem1024_avx2.M(Syscall)._poly_tobytes :
...
+ auto => /> &1 &2. 
  ...
  rewrite !get_storesE !size_take 1:/# !size_to_list !iotaredE /max /=.
...
seq 36 36 : (#pre);  1: by conseq />;sim.
auto => /> &1 &2.
rewrite /storeW256 !of_uintK !modz_small; 1..6: by auto => /#. 
apply JMemory.mem_eq_ext => p. 
rewrite !get_storesE !size_to_list !size_take 1:/# !size_to_list !iotaredE /max /=.
...
lemma polyvec_to_bytes_stack_equiv _mem _pos:
   0 <= _pos <= 1568 =>
   equiv [ Jkem1024_avx2_stack.M.__i_polyvec_tobytes
           ~ Jkem1024_avx2.M(Syscall).__polyvec_tobytes :
...
seq 3 3 : (
Glob.mem{2} = (stores _mem _pos (take (4*384) (to_list r{1}))) /\ pp{2} = (of_int (_pos + 3*384))%W64 /\ ={a}
).
+ exlim Glob.mem{2}, a{1} => _mem4 _a4.
  wp;call (poly_to_bytes_stack_equiv _mem4 (_pos+3*384) ((init (fun (i : int) => _a4.[768 + i]))%Array256)) => /=;1: by smt().
  auto => /> &1  rrl;apply mem_eq_ext => p.
  rewrite !get_storesE !size_to_list !size_take 1..2:/# !size_to_list /=. 
  case (_pos + 1152 <= p < _pos + 1536) => H.
  + rewrite ifT 1:/#. 
    rewrite nth_take 1,2:/# -!(nth_change_dfl W8.zero witness) ?size_to_list 1,2:/#.
    by rewrite !get_to_list initiE /#.
  case (_pos <= p < _pos + 1152) => H1.
  + rewrite ifT 1:/#. 
    rewrite !nth_take 1..4:/# -!(nth_change_dfl W8.zero witness) ?size_take ?size_to_list 1..2:/#.
    by rewrite !get_to_list initiE /#.
  + by smt()
...
from JazzEC require import WArray1568 Array192.

lemma copy1536 (a : W8.t Array1536.t) :
Array1536.init (fun (i : int)  =>  WArray1536.get8 (WArray1536.init64                                                                       
               (fun (i0 : int) =>  copy_64 (Array192.init                                                                             
               (fun (i1 : int) =>  WArray1536.get64 (WArray1536.init8                                                                  
...
lemma poly_from_bytes_stack_equiv _pos _mem :
   0 <= _pos <= 1152 =>
   equiv [ Jkem1024_avx2_stack.M._i_poly_frombytes
           ~ Jkem1024_avx2.M(Syscall)._poly_frombytes :
...
lemma polyvec_from_bytes_stack_equiv:
   equiv [ Jkem1024_avx2_stack.M.__i_polyvec_frombytes
           ~ Jkem1024_avx2.M(Syscall).__polyvec_frombytes :
   to_uint arg{2} = 0 /\
   load_array1536 Glob.mem{2} 0 = arg{1} ==> ={res}].
proc => //.
proc rewrite {1} 3 copy1536.
unroll for {1} 5; unroll for {2} 4.
exlim Glob.mem{2} => _mem.
seq 5 4 : (={r} /\ to_uint pp{2} = 0 /\ Glob.mem{2} = _mem /\ load_array1536 Glob.mem{2} 0 = a{1}
).
...
seq 2 3 : (={r} /\ to_uint pp{2} = 384 /\ Glob.mem{2} = _mem  /\ load_array1536 Glob.mem{2} 0 = a{1}).
...
seq 2 3 : (
={r} /\ to_uint pp{2} = 768 /\ Glob.mem{2} = _mem  /\ load_array1536 Glob.mem{2} 0 = a{1}).
...
seq 2 3 : (
={r} /\ to_uint pp{2} = 1152 /\ Glob.mem{2} = _mem  /\ load_array1536 Glob.mem{2} 0 = a{1}).
+ wp;call (poly_from_bytes_stack_equiv 1152 _mem) => /=. 
  auto => /> &1 Hpp;rewrite !to_uintD_small /= 1:/# /= Hpp /=.
  rewrite /load_array384 /load_array1536 tP => k kb.
  by rewrite !initiE 1,2:/# /= initiE /#.

by auto => />;smt(Array1024.tP).
qed.

from JazzEC require import WArray1568.

lemma polyvec_decompress_stack_equiv:
   equiv [ Jkem1024_avx2_stack.M.__i_polyvec_decompress
           ~ Jkem1024_avx2.M(Syscall).__polyvec_decompress :
   to_uint arg{2} = 1536 /\
   load_array1568 Glob.mem{2} 1536 = arg{1} ==> ={res}].
proc.
admitted.

from JazzEC require import WArray160 WArray512.
lemma poly_decompress_stack_equiv:
   equiv [ Jkem1024_avx2_stack.M._i_poly_decompress
           ~ Jkem1024_avx2.M(Syscall)._poly_decompress :
   to_uint arg{2}.`2 = 1536+1408 /\
   load_array160 Glob.mem{2} (1536+1408) = arg{1}.`2 ==> ={res}].
admitted.
