...
from JazzEC require import Array16 Array32 Array64 Array128 Array160 Array168 Array256 Array384 Array1024 Array1408 Array1536.
...
module EncDec_AVX2 = {
  ...
  proc decode11_opt_vec(u: W8.t Array1408.t) : ipolyvec = {
    (* 
    var c : ipolyvec;
    var k;
  ...
  proc encode12_opt_vec(a : ipolyvec) : W8.t Array1536.t = {
    var a1, a2, a3, a4;
    a1 <@ encode12_opt(subarray256 a 0);
    a2 <@ encode12_opt(subarray256 a 1);
    a3 <@ encode12_opt(subarray256 a 2);
    a4 <@ encode12_opt(subarray256 a 3);
    return fromarray384 a1 a2 a3 a4;
  }

  proc encode11_vec(a: ipolyvec) : W8.t Array1408.t = {
    (* 
  ...
  proc encode11_opt_vec(a : ipolyvec) : W8.t Array1408.t = {
    (* 
    var c : W8.t Array960.t;
    var i;
...
module MLKEM_avx2_encdec = {
  ...
  proc __red_x16 (r: W16.t Array16.t) : W16.t Array16.t = {
    ...
    i <- 0;
    while(i < 16) {
      t <@Jkem1024.M(Jkem1024.Syscall).__barrett_reduce(r.[i]);
      r.[i] <- t;
      i <- i + 1;
    }
...
equiv decode11_vec_corr:
  EncDec_AVX2.decode11_vec ~ EncDec.decode11_vec: ={u} ==> ={res}.
admitted. (* 
proof.
  proc.
  swap 2 1.
...
equiv eq_decode11_opt_vec:
  EncDec_AVX2.decode11_opt_vec ~ EncDec_AVX2.decode11_vec: ={u} ==> ={res}.
admitted.
(* 
proof.
  proc.
...
equiv encode11_vec_corr:
  EncDec_AVX2.encode11_vec ~ EncDec.encode11_vec: a{1} = u{2} ==> ={res}.
admitted. (* 
proof.
  proc.
  swap 2 1.
...
equiv eq_encode11_opt_vec:
  EncDec_AVX2.encode11_opt_vec ~ EncDec_AVX2.encode11_vec: ={a} ==> ={res}.
admitted.
(* 
proof.
  proc.
...
equiv encode11_opt_corr:
  EncDec_AVX2.encode11_opt_vec ~ EncDec.encode11_vec: a{1} = u{2} ==> ={res}.
admitted. (* 
proof.
...
equiv eq_decode5:
  EncDec_AVX2.decode5 ~ EncDec.decode5: ={a} ==> ={res}.
admitted.
(* 
proc.
unroll for {1} ^while.
...
equiv eq_encode5:
  EncDec_AVX2.encode5 ~ EncDec.encode5: ={p} ==> ={res}.
admitted.
(* 
proof.
  proc.
