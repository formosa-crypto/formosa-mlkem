...
lemma poly_decompress_corr mem _p (_a : W8.t Array160.t): 
    equiv [ Mprevec.poly_decompress ~ EncDec_AVX2.decode5 :
            valid_ptr _p 160 /\
    ...
    + inline *. wp. skip.
      ...
      do split.
        ...
        + move => k k_lb k_ub.
          rewrite /(pd_mask_s) /=.
          do (rewrite initiE 1:/# /=).
          rewrite /(Jkem1024_avx2.pc_mask_s) /of_int.
          ...
          case (k %% 2 = 0) => k_even.
            ...
            + do (rewrite initiE 1:/# /=).
              smt().
        + move => k k_lb k_ub.
          rewrite /(Jkem1024_avx2.pd_shift_s) /=.
          do (rewrite initiE 1:/# /=).
          rewrite /of_int.
          apply W16.wordP => x x_i.
          rewrite bits16E initiE 1:x_i /=.
          ...
          case (k %% 2 = 0) => k_even.
            ...
            + do (rewrite initiE 1:/# /=).
              smt().
        + move => k k_lb k_ub.
          rewrite /(Jkem1024_avx2.pd_jshufbidx) /=.
          do (rewrite initiE 1:/# /=).
          smt().
...
lemma poly_compress_1_corr _a mem :
    equiv [ Mprevec.poly_compress_1 ~ EncDec_AVX2.encode5 :
...
proof.
  ...
  do split.
    ...
    + move => k k_lb k_ub.
      ...
      do (rewrite initiE 1:/# /=).
      rewrite shlMP 1:/# /=.
      rewrite /(Jkem1024_avx2.pc_shift2_s) /of_int.
      ...
      case (k %% 2 = 0) => k_even.
        ...
        + have ->: (k %% 2 = 1). move : k_even => /#.
          ...
          smt().
    + move => k k_lb k_ub.
      rewrite /(Jkem1024_avx2.pc_permidx_s) /=.
      do (rewrite initiE 1:/# /=).
      smt().
    wp.
...
lemma poly_compress_corr _a (_p : address) mem :
    equiv [ Mprevec.poly_compress ~ EncDec_AVX2.encode5 :
             pos_bound256_cxq a{1} 0 256 2 /\
             lift_array256 a{1} = _a /\
             p{2} = compress_poly 5 _a /\
             valid_ptr _p 160 /\
...
proof.
  ...
  split.
    + move => k k_lb k_ub.
      rewrite initiE 1:/# //=.
      rewrite /(Jkem1024_avx2.hqx16_m1) initiE 1:/# //=.
      smt().
    + move => k k_lb k_ub.
      rewrite initiE 1:/# //=.
      rewrite /(Jkem1024_avx2.hhqx16) initiE 1:/# //=.
      smt().
  wp.
...
proof. 
  ...
  have idx_in_bound: forall j, 0 <= j < 32 => ! 128 <= to_uint idx{1}.[j].
    ...
    rewrite /(Jkem1024_avx2.pfm_idx_s) initiE //= 1:/#.
  ...
  rewrite (_: (W2u8.Pack.init (fun (j: int) => (Array32.init (fun (i : int) =>
    ...
    rewrite (pmod_small _ 16) //=.
      rewrite idx_def 1:/# /(Jkem1024_avx2.pfm_idx_s) initiE //= 1:/#.
      do (rewrite (fun_if W8.to_uint) /=).
      smt().
