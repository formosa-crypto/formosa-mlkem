...
module Mprevec = {
  ...
  proc polyvec_frombytes (ap:W64.t) : W16.t Array1024.t = {
    ...
    pp <- (pp + (W64.of_int 384));
    aux <@ MLKEM_Poly_avx2_prevec.Mprevec.poly_frombytes ((Array256.init (fun i => r.[(3 * 256) + i])),
    pp);
    r <- Array1024.init
         (fun i => if (3 * 256) <= i < (3 * 256) + 256 then aux.[i-(3 * 256)]
         else r.[i]);
  ...
  proc polyvec_tobytes (rp:W64.t, a:W16.t Array1024.t) : unit = {
    ...
    pp <- (pp + (W64.of_int 384));
    aux <@ MLKEM_Poly_avx2_prevec.Mprevec.poly_tobytes (pp, (Array256.init (fun i => a.[(3 * 256) + i])));
    a <- Array1024.init
         (fun i => if (2 * 256) <= i < (3 * 256) + 256 then aux.[i-(3 * 256)]
         else a.[i]);
    return ();
  }

  proc polyvec_compress_1 (rp:W8.t Array1408.t, a:W16.t Array1024.t) : W8.t Array1408.t = {
    var aux: int;
    ...
    while (i < aux) {
      ...
      rp <- Array1408.fill (fun j => t0.[j %% 20]) (20*i) 16 rp;
      t0_d <@ Ops.iVPEXTR_32(t1, (W8.of_int 0));
      rp <- Array1408.fill (fun j => t0_d.[j %% 4]) (20*i + 16) 4 rp;
      i <- i + 1;
    }
    return (rp);
  }
  ...
  proc polyvec_decompress (rp:W64.t) : W16.t Array1024.t = {
    ...
    while (k < 3) {
      ...
      while (i < aux) {
        ...
        r <- Array1024.fill (fun j => f.[j %% 16]) (256 * k + 16 * i) 16 r;
        i <- i + 1;
      }
      k <- k + 1;
    }
